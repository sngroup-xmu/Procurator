...
COPY docs/ docs/
# Narrow copies let toolchain/solver layers stay cached when only scripts
# or docs under artifact/ change.
...
WORKDIR /procurator
COPY --from=build /procurator/src/bin src/bin
COPY --from=build /procurator/src/dslc src/dslc
COPY --from=build /procurator/src/p4b/python src/p4b/python
COPY --from=build /procurator/src/p4b/source/p4include src/p4b/source/p4include
...
COPY --from=build /procurator/benchmarks benchmarks
COPY --from=build /procurator/artifact artifact
COPY --from=build /procurator/docs docs
