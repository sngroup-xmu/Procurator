...
class TestBoogieBackendSmoke(unittest.TestCase):
    ...
    def _native_register_bpl(*, include_marker: bool = True, include_mirror_modifies: bool = True) -> str:
        ...
        if include_mirror_modifies:
            modifies += (
                ", my_reg__last_index, my_reg__last_value, my_reg__wrote_any, "
                "my_reg__wrote_index0, my_reg__last0_value"
            )
        return f"""\
type Ref;
...
"""
    ...
    def test_legacy_bpl_native_register_mirrors_are_not_reinstrumented_after_prefixing(self) -> None:
        # Legacy .bpl imports may already contain native P4B-style mirrors. The
        # compatibility backfill should recognize them and stay idempotent.
        bpl_text = """\
type Ref;
var standard_metadata.egress_port:bv9;
var hdr.ipv4:Ref;
...
procedure {:inline 1} my_reg.write(index:bv32, value:bv32)
  modifies my_reg, my_reg__last_index, my_reg__last_value, my_reg__wrote_any, my_reg__wrote_index0, my_reg__last0_value;
{
  my_reg[index] := value;
  my_reg__last_index := index;
  my_reg__last_value := value;
...
"""
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            raw_bpl = td_path / "prog.bpl"
            raw_bpl.write_text(bpl_text, encoding="utf-8")
            text = self._compile_single_node_bpl(raw_bpl)

        self.assertIn(
            "modifies s1_my_reg, s1_my_reg__last_index, s1_my_reg__last_value, "
            "s1_my_reg__wrote_any, s1_my_reg__wrote_index0, s1_my_reg__last0_value;",
            text,
        )
    ...
    def test_p4b_generated_nodes_must_emit_register_mirrors(self) -> None:
        # P4B-generated nodes are not legacy BPL imports. If register mirror
        # semantics are missing, DSLC must fail instead of patching P4-local
        # semantics from the distributed harness side.
...
"""
    ...
    def test_p4b_generated_nodes_accept_complete_native_register_mirrors(self) -> None:
        raw_bpl_with_mirrors = self._native_register_bpl()
...
"""
    ...
    def test_p4b_generated_nodes_reject_incomplete_mirror_modifies(self) -> None:
        raw_bpl_without_modifies = self._native_register_bpl(include_mirror_modifies=False)
