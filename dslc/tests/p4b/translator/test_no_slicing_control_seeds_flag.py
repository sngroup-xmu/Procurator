...
class TestP4BTranslatorNoSlicingControlSeedsFlag(unittest.TestCase):
    def _compile_with(self, *, keep_control_seeds: bool) -> list[str]:
        with tempfile.TemporaryDirectory() as td:
            ...
            with mock.patch("dslc.backends.boogie_p4b.subprocess.run", side_effect=fake_run):
                P4BTranslator(p4b_bin="/bin/true", include_paths=[]).compile_to_bpl(
                    str(p4),
                    str(out_bpl),
                    entries_path=None,
            ...
            return list(seen.get("cmd") or [])

    def test_flag_added_when_disabled(self) -> None:
        self.assertIn("--no-slicing-control-seeds", self._compile_with(keep_control_seeds=False))

    def test_flag_not_added_by_default(self) -> None:
        self.assertNotIn("--no-slicing-control-seeds", self._compile_with(keep_control_seeds=True))
