        ...
        */ 
            ...
            */
                ...
                RegisterAction<queue_len_t, _, queue_len_t>(deferred_queue_len_list_1) check_deferred_queue_len_list_1 = {
                    void apply(inout queue_len_t value, out queue_len_t rv) {
                        if (value <= ig_md.queue_len_diff) { // Queue len drift is not large enough to invalidate the decision
                            ...
                            rv = value + ig_md.selected_ds_qlen; // to avoid using another stage for this calculation
                        }
                    }
                };
            ...
            Register<queue_len_t, _>(MAX_WORKERS_IN_RACK) deferred_queue_len_list_2; // List of queue lens for all vclusters
                ...
                RegisterAction<queue_len_t, _, queue_len_t>(deferred_queue_len_list_2) read_deferred_queue_len_list_2 = {
                    void apply(inout queue_len_t value, out queue_len_t rv) {
                        rv = value + ig_md.not_selected_ds_qlen;
                    }
                };
             ...
             * In summary, we use ig_md.queue_len_unit which is the fixed point representation for 1/#workers in rack,
                ...
                RegisterAction<bit<16>, _, bit<16>>(aggregate_queue_len_list) update_read_aggregate_queue_len = {
                    void apply(inout bit<16> value, out bit<16> rv) {
                        if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                            value = value + ig_md.queue_len_unit;
                        } else {
                            if (value >= ig_md.queue_len_unit) {
                               value = value - ig_md.queue_len_unit; 
                            }
                        }
                        rv = value;
            ...
            */
            Register<bit<16>, _>(MAX_VCLUSTERS) linked_view_drift; 
                RegisterAction<bit<16>, _, bit<16>>(linked_view_drift) inc_read_linked_view_drift  = {
                    void apply(inout bit<16> value, out bit<16> rv) {
                        if (value == ig_md.cluster_num_valid_ds - 1) {
                            rv = 0;
                            value = 0;
                        } else {
                            rv = 1;
            ...
            Register<bit<16>, _>(MAX_VCLUSTERS) idle_link_spine_view; 
                ...
                RegisterAction<bit<16>, _, bit<16>>(idle_link_spine_view) read_idle_link_spine_view  = {
                    void apply(inout bit<16> value, out bit<16> rv) {
                        rv = value;
                        if (value == 1 && ig_md.cluster_idle_count == 0){
                            value = 0;
                        } else {
                            value = 1;
                        }
            ...
            Register<bit<16>, _>(MAX_VCLUSTERS) backoff3; 
                RegisterAction<bit<16>, _, bit<16>>(backoff3) inc_read_backoff3  = {
                    void apply(inout bit<16> value, out bit<16> rv) {
                        if (value == ig_md.cluster_num_valid_ds) {
                            rv = 0;
                            value = 0;
                        } else {
                            rv = 1;
            ...
            */ 
            action act_set_queue_len_unit(len_fixed_point_t cluster_unit) {
                ig_md.queue_len_unit = cluster_unit;
            }
            ...
            */
            action gen_random_probe_group() { 
                ...
                */
                ...
                ig_intr_tm_md.mcast_grp_b = ig_md.random_id_1; 
            }
            ...
            */
            action act_get_cluster_num_valid(bit<16> num_ds_elements, bit<16> num_us_elements) {
                ig_md.cluster_num_valid_ds = num_ds_elements;
                ig_md.cluster_num_valid_us = num_us_elements;
            }
             ...
             * E.g Key: ig_md.cluster_num_valid_us=16. Action: adjust_random_worker_range_4 (shift 12 bits)
            */
            action gen_random_workers_16() { // Random for 65536 Nodes
                ig_md.random_id_1 = (bit<16>) random_worker_id_16.get();
                ig_md.random_id_2 = (bit<16>) random_worker_id_16.get();
            }
            
            action adjust_random_worker_range_8() { // Random for 256 Nodes
                ig_md.random_id_1 = ig_md.random_id_1 >> 8;
                ig_md.random_id_2 = ig_md.random_id_2 >> 8;
            }

             action adjust_random_worker_range_5() { // Random for 16 Nodes
                ig_md.random_id_1 = ig_md.random_id_1 >> 11;
                ig_md.random_id_2 = ig_md.random_id_2 >> 11;
            }

            action adjust_random_worker_range_4() { // Random for 16 Nodes
                ig_md.random_id_1 = ig_md.random_id_1 >> 12;
                ig_md.random_id_2 = ig_md.random_id_2 >> 12;
            }

            action adjust_random_worker_range_3() { // Random for 8 Nodes
                ig_md.random_id_1 = ig_md.random_id_1 >> 13;
                ig_md.random_id_2 = ig_md.random_id_2 >> 13;
            }

            action adjust_random_worker_range_2() { // Random for 4 Nodes
                ig_md.random_id_1 = ig_md.random_id_1 >> 14;
                ig_md.random_id_2 = ig_md.random_id_2 >> 14;
            }

            action adjust_random_worker_range_1() { // Random for 2 Nodes
                ig_md.random_id_1 = ig_md.random_id_1 >> 15;
                ig_md.random_id_2 = ig_md.random_id_2 >> 15;
            }

            table adjust_random_range_ds { // Reduce the random generated number (16 bit) based on number of workers in rack
                key = {
                    ig_md.cluster_num_valid_ds: exact; 
                }
                actions = {
            ...
            */
            table adjust_random_range_us { // Reduce the random generated number (16 bit) based on number of workers in rack
                key = {
                    ig_md.cluster_num_valid_us: exact; 
                }
                actions = {
            ...
            */
            action act_get_spine_dst_id(bit <16> spine_dst_id){
                ig_md.spine_to_link_iq = spine_dst_id;
            }
            ...
            action offset_random_ids() {
                ig_md.random_id_1 = ig_md.random_id_1 + ig_md.cluster_ds_start_idx;
                ig_md.random_id_2 = ig_md.random_id_2 + ig_md.cluster_ds_start_idx;
            }
            
            
            action dec_repeated_rand() { // Used for breaking ties when two randomly generated numbers point to same index
                ig_md.random_id_2 = ig_md.random_id_2-1;
            }
                    ...
                    if (ig_intr_md.resubmit_flag != 0) { // Special case: packet is resubmitted just update the indexes
                        ...
                        @stage(1){
                            if (ig_md.min_correct_qlen == ig_md.task_resub_hdr.qlen_1) {
                                hdr.horus.dst_id = ig_md.task_resub_hdr.ds_index_1;
                                ig_md.selected_ds_qlen = ig_md.task_resub_hdr.qlen_1 + 1;
                            } else {
                                hdr.horus.dst_id = ig_md.task_resub_hdr.ds_index_2;
                                ig_md.selected_ds_qlen = ig_md.task_resub_hdr.qlen_2 + 1;
                            }
                        }
                        @stage(5) {
                        ...
                        if (hdr.horus.pkt_type == PKT_TYPE_TASK_DONE_IDLE) {  // worker reply and its IDLE, read increment counter (pointer to idle list)
                            ig_md.cluster_idle_count = read_and_inc_idle_count.execute(hdr.horus.cluster_id);
                            ...
                            ig_md.cluster_idle_count = read_and_dec_idle_count.execute(hdr.horus.cluster_id); // Read last idle count for vcluster
                            ...
                            ig_md.cluster_idle_count = read_idle_count.execute(hdr.horus.cluster_id);
                        ...
                        if (hdr.horus.pkt_type == PKT_TYPE_QUEUE_REMOVE) { 
                            ...
                            ig_md.linked_sq_id = read_update_linked_sq.execute(hdr.horus.cluster_id); // Get ID of the Spine that the leaf reports to   
                        }
                        ...
                        @stage(1) {
                            ...
                            if (hdr.horus.pkt_type == PKT_TYPE_TASK_DONE_IDLE || hdr.horus.pkt_type == PKT_TYPE_TASK_DONE || hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                                ig_md.aggregate_queue_len = update_read_aggregate_queue_len.execute(hdr.horus.cluster_id);
                            } 
                        ...
                        @stage(2) {
                            ...
                            ig_md.received_dst_id = hdr.horus.dst_id; //  Keep received dst_id so that we can swap src_id dst_id to reply to other switches
                            if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK){
                                ig_md.num_valid_half = ig_md.cluster_num_valid_ds >> 1;
                        ...
                        @stage(3) {
                            if (hdr.horus.pkt_type == PKT_TYPE_TASK_DONE_IDLE || hdr.horus.pkt_type == PKT_TYPE_TASK_DONE) { 
                                ...
                                if (hdr.horus.pkt_type == PKT_TYPE_TASK_DONE_IDLE) { 
                                    add_to_idle_list.execute(ig_md.idle_ds_index); // If worker reply and its idle, add it to idle list
                                }
                                ...
                                if (hdr.horus.qlen == 1) { // Spine thinks this leaf is idle
                                    ig_md.backoff_counter1 = inc_read_backoff1.execute(hdr.horus.cluster_id);
                                    
                                } else {
                                    reset_backoff1.execute(hdr.horus.cluster_id);
                                }
                                if (ig_md.cluster_idle_count > 0) { // If a new task arrives and idle workers available read idle_list to get ID of idle worker
                                    ig_md.idle_ds_id = read_idle_list.execute(ig_md.idle_ds_index); 
                                } else {
                                    if(ig_md.random_id_1 == ig_md.random_id_2) { 
                                        if (ig_md.random_id_2 == 0){ // Break ties in case two random numbers point to same index
                                            inc_repeated_rand();
                                        } else {
                                            dec_repeated_rand();
                                        }
                        ...
                        @stage(4) { 
                            if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK ){
                                ...
                                ig_md.backoff_counter3 = inc_read_backoff3.execute(hdr.horus.cluster_id);
                            }
                        }
                        ...
                        @stage(5) {
                            if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                                ...
                                ig_md.random_ds_qlen_1 = read_queue_len_list_1.execute(ig_md.random_id_1);
                                ig_md.random_ds_qlen_2 = read_queue_len_list_2.execute(ig_md.random_id_2);
                                ...
                                */
                                if (hdr.horus.qlen == 1 && ig_md.backoff_counter1 == 0) { // Spine thinks this rack is idle so update the idle linkage (leaf is still linked)
                                ...
                                if (ig_md.cluster_idle_count > 1 && ig_md.backoff_counter3 ==0) { 
                                    ...
                                    ig_md.idle_link = write_linked_iq.execute(hdr.horus.cluster_id);
                                } else if (ig_md.cluster_idle_count <= 1) {
                                    // If the idle workers are not available, reset the linkage (read previous value so that we send idleRemove in next stage)
                                    ig_md.idle_link = read_linked_iq.execute(hdr.horus.cluster_id); 
                                } 
                            }  
                        }
                        ...
                        @stage(6) {
                            if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                                ...
                                if (ig_md.cluster_idle_count <= 1) { // Breaking the linkage
                                    if (ig_md.idle_link != INVALID_VALUE_16bit) { // Send a IDLE_REMOVE packet to the linked spine, mirror original reply to client
                                        hdr.horus.dst_id = ig_md.idle_link; // Send idle remove packet to the linked spine
                                        /* 
                                        ...
                                        */
                                        ...
                                        hdr.horus.qlen = ig_md.aggregate_queue_len; 
                                    ...
                                    if(ig_md.idle_link == INVALID_VALUE_16bit) { // Send IDLE_SIGNAL to the linked leaf if previously not linked with a spine
                                        /* 
                                         * TESTBEDONLY: Here we should set the ID of the leaf switch as src_id, 
                                          but in our experimetns we used cluster_ids to diffrentiate the emulated leaf switches.
                                          So we use the cluster_id when sending reply to spine. 
                                          In real world it should be a constant SWITCH_ID. 
                                        */
                                        hdr.horus.dst_id = ig_md.spine_to_link_iq;
                                        hdr.horus.qlen = ig_md.aggregate_queue_len;
                                        //hdr.horus.src_id = SWITCH_ID; 
                        ...
                        @stage(7) {
                            ...
                            if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                                if (ig_md.cluster_idle_count == 0) {
                                    if (ig_md.selected_ds_qlen == ig_md.random_ds_qlen_1) { // If minimum qlen belongs to our first sample
                                        hdr.horus.dst_id = ig_md.random_id_1; // Set dst_id to id of first sample
                                        // line below is used in case of resubmission, we always keep the id of *not selected* in task_resub_hdr.ds_index_2
                                        ig_md.task_resub_hdr.ds_index_2 = ig_md.random_id_2; 
                                    } else { // If minimum qlen belongs to our second sample
                                        hdr.horus.dst_id = ig_md.random_id_2;
                                        ig_md.task_resub_hdr.ds_index_2 = ig_md.random_id_1;
                                    }
                                ...
                                ig_md.spine_view_ok = inc_read_linked_view_drift.execute(hdr.horus.cluster_id); // Check if drift of load of the spine is larger than threshold
                            } 
                        }
                        ...
                        @stage(8) {
                            ...
                            } else if(hdr.horus.pkt_type==PKT_TYPE_NEW_TASK && ig_md.cluster_idle_count == 0) {
                                if (ig_md.random_id_2 != ig_md.random_id_1) {
                                    ...
                                    ig_md.task_resub_hdr.qlen_1 = check_deferred_queue_len_list_1.execute(hdr.horus.dst_id); 
                                    // keep the ID of selected dst in ds_index_1
                                    ig_md.task_resub_hdr.ds_index_1 = hdr.horus.dst_id;
                        ...
                        @stage(9){
                            if (hdr.horus.pkt_type==PKT_TYPE_TASK_DONE_IDLE || hdr.horus.pkt_type==PKT_TYPE_TASK_DONE) {
                                ...
                                if (ig_md.spine_view_ok == 0) { // Need to send a new load signal to spine 
                                    /* 
                                    ...
                                    */
                                    ...
                                    hdr.horus.dst_id = ig_md.linked_sq_id;
                                    ig_intr_dprsr_md.mirror_type = MIRROR_TYPE_WORKER_RESPONSE; 
                                }
                            } else if(hdr.horus.pkt_type==PKT_TYPE_NEW_TASK) {
                                if (ig_md.cluster_idle_count == 0){
                                    if(ig_md.task_resub_hdr.qlen_1 == 0) { // This return value means that we do not need to check deffered qlens, difference between samples were larger than drift so our decision is still valid
    ...
    apply {
        if (ig_intr_dprsr_md.mirror_type == MIRROR_TYPE_WORKER_RESPONSE) {
            ...
            */
            ...
            mirror.emit(ig_md.mirror_dst_id); 
        }  
