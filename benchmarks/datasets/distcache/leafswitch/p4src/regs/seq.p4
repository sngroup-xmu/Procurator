...
action assign_seq() {
	...
	seq_reg.read(seq_res,(bit<32>)hdr_eg.inswitch_hdr.hashval_for_seq);
	if(seq_res == 0x7FFFFFFF){
		hdr_eg.seq_hdr.seq=0;
		seq_res =0;
	}
	else{
		hdr_eg.seq_hdr.seq=seq_res+1;
		seq_res= seq_res+1;
	}
	seq_reg.write((bit<32>)hdr_eg.inswitch_hdr.hashval_for_seq,seq_res);
}
...
table access_seq_tbl {
	key = {
		...
		hdr_eg.fraginfo_hdr.cur_fragidx: exact;
	}
	actions = {
		assign_seq;
		NoAction;
...
action get_savedseq() {
	...
	savedseq_reg.read(hdr_eg.seq_hdr.seq,(bit<32>)hdr_eg.inswitch_hdr.idx);
}


action set_and_get_savedseq() {
	// set_and_get_savedseq_alu.execute_stateful_alu(hdr.inswitch_hdr.idx);
	savedseq_reg.write((bit<32>)hdr_eg.inswitch_hdr.idx,hdr_eg.seq_hdr.seq);
}
