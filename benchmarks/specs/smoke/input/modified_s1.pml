...
proctype s1() {
  ...
  int SteamCounter = 0;
  int PacketCounter = 0;
  int BufferUsage = (((PacketCounter + SteamCounter) + 1) * ((PacketCounter + SteamCounter) + 1));
    ...
    if
      :: egress_port == 1 ->
          s2_in!packet;
      :: else ->
          printf("Unknown egress_port %d in D:\work\P4-verification\code\spec\test\input\modified_s1.pml\n", egress_port);
    fi;

    // Assertions
    assert((BufferUsage < 100));
    assert((PacketCounter <= 1000));

    // LTL Statements
    ltl: (false || (true && [](<>((BufferUsage < 90)))));
    ltl: (false || (true && [](<>((PacketCounter <= 900)))));
od
