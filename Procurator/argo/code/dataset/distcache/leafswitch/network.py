...
def create_network():
    ...
    for i in range(rack_physical_num):
        spineswitchoses.append(
            net.addHost(
                "spines{}".format(1 + i),
                ip="192.168.1.{}/24".format(100 + i + 1),
                mac="00:00:0a:00:01:{}".format(hex(100 + i + 1)[2:]),
            )
        )
    ...
    # fully connected
    for i in range(rack_physical_num):
        net.addLink(client_s1, spineswitchs[i])
        for j in range(rack_physical_num):
            net.addLink(spineswitchs[i], rackswitchs[j])
    ...
    for i in range(rack_physical_num):
        ...
        net.addLink(spineswitchoses[i], nat_s1)
