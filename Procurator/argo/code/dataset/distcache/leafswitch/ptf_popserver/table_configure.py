...
class RegisterUpdate():
    ...
    def add_cache_lookup(
        self, keylolo, keylohi, keyhilo, keyhihilo, keyhihihi, freeidx
    ):
        ...
        while True:
            ...
            if control_type == SWITCHOS_ADD_CACHE_LOOKUP:
                ...
                (
                    ...
                    keyhihihi,
                    recvbuf,
                ) = struct.unpack("!3I2H{}s".format(len(recvbuf) - 16), recvbuf)
                freeidx = struct.unpack("=H", recvbuf)[0]
                self.add_cache_lookup(
                    keylolo, keylohi, keyhilo, keyhihilo, keyhihihi, freeidx
                )
                ...
                # parse key
                keylolo, keylohi, keyhilo, keyhihilo, keyhihihi = struct.unpack(
                    "!3I2H", recvbuf
                )
                # remove key from cache_lookup_tbl
                self.remove_cache_lookup(
                    keylolo, keylohi, keyhilo, keyhihilo, keyhihihi
                )
