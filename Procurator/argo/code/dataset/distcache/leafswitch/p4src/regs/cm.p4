...
action update_cm1() {
	// update_cm1_alu.execute_stateful_alu(hdr_eg.inswitch_hdr.hashval_for_cm1);
	cm1_reg.read(cm1_res,(bit<32>)hdr_eg.inswitch_hdr.hashval_for_cm1);
	meta.cm1_predicate = 1;
	if(cm1_res >= hdr_eg.inswitch_hdr.hot_threshold){
		meta.cm1_predicate = 2;
	}
	cm1_reg.write((bit<32>)hdr_eg.inswitch_hdr.hashval_for_cm1,cm1_res+1);
}
...
table access_cm1_tbl {
	key = {
		...
		hdr_eg.inswitch_hdr.is_sampled: exact;
...
action update_cm2() {
	...
	cm2_reg.read(cm2_res,(bit<32>)hdr_eg.inswitch_hdr.hashval_for_cm2);
	meta.cm2_predicate = 1;
	if(cm2_res >= hdr_eg.inswitch_hdr.hot_threshold){
		meta.cm2_predicate = 2;
	}
	cm2_reg.write((bit<32>)hdr_eg.inswitch_hdr.hashval_for_cm2,cm2_res+1);
}
...
action update_cm3() {
	cm3_reg.read(cm3_res,(bit<32>)hdr_eg.inswitch_hdr.hashval_for_cm3);
	meta.cm3_predicate = 1;
	if(cm3_res >= hdr_eg.inswitch_hdr.hot_threshold){
		meta.cm3_predicate = 2;
	}
	cm1_reg.write((bit<32>)hdr_eg.inswitch_hdr.hashval_for_cm3,cm3_res+1);
}
...
action update_cm4() {
	cm4_reg.read(cm4_res,(bit<32>)hdr_eg.inswitch_hdr.hashval_for_cm4);
	meta.cm4_predicate = 1;
	if(cm4_res >= hdr_eg.inswitch_hdr.hot_threshold){
		meta.cm4_predicate = 2;
	}
	cm1_reg.write((bit<32>)hdr_eg.inswitch_hdr.hashval_for_cm4,cm4_res+1);
}
