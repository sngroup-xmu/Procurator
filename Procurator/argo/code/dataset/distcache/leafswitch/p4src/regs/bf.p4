...
action update_bf1() {
	bf1_reg.write((bit<32>)hdr_eg.inswitch_hdr.hashval_for_bf1,1);
	meta.is_report1 = 1;
}
...
action update_bf2() {
	// update_bf2_alu.execute_stateful_alu(hdr_eg.inswitch_hdr.hashval_for_bf2);
	bf2_reg.write((bit<32>)hdr_eg.inswitch_hdr.hashval_for_bf2,1);
	meta.is_report2 = 1;
}
...
action update_bf3() {
	// update_bf3_alu.execute_stateful_alu(hdr_eg.inswitch_hdr.hashval_for_bf3);
	bf3_reg.write((bit<32>)hdr_eg.inswitch_hdr.hashval_for_bf3,1);
	meta.is_report3 = 1;
}
