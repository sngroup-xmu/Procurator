...
control netcacheEgress (inout headers hdr_eg,
                 inout metadata meta,
	...
	action save_client_udpport() {
		hdr_eg.clone_hdr.client_udpport = hdr_eg.udp_hdr.srcPort;
	}
	...
	action set_server_sid_and_port(bit<10> server_sid) {
		...
		hdr_eg.clone_hdr.server_sid = server_sid;
		hdr_eg.clone_hdr.server_udpport = hdr_eg.udp_hdr.dstPort; // dstport is serverport for GETREQ_INSWITCH
		hdr_eg.clone_hdr.setValid();
	}

	action reset_server_sid() {
		hdr_eg.clone_hdr.server_sid = 0;
	}
	...
	action set_is_hot() {
		meta.is_hot = 1;
		//hdr_eg.debug_hdr.is_hot = 1;
	}

	action reset_is_hot() {
		meta.is_hot = 0;
		//hdr_eg.debug_hdr.is_hot = 0;
	}
	...
	action set_is_lastclone() {
		...
		//hdr_eg.debug_hdr.is_lastclone_for_pktloss = 1;
	}

	action reset_is_lastclone_lastscansplit() {
		meta.is_lastclone_for_pktloss = 0;
		//hdr_eg.debug_hdr.is_lastclone_for_pktloss = 0;
	}
	...
	table lastclone_lastscansplit_tbl {
		key = {
			hdr_eg.op_hdr.optype: exact;
			hdr_eg.clone_hdr.clonenum_for_pktloss: exact;
		}
		actions = {
			set_is_lastclone;
	...
	action reverse_ip(){
		bit<32> tmp_ip;
		tmp_ip = hdr_eg.ipv4_hdr.srcAddr;
		hdr_eg.ipv4_hdr.srcAddr = hdr_eg.ipv4_hdr.dstAddr;
		hdr_eg.ipv4_hdr.dstAddr = tmp_ip;
	}
	action reverse_mac(){
		bit<48> tmp_mac;
		tmp_mac = hdr_eg.ethernet_hdr.srcAddr;
		hdr_eg.ethernet_hdr.srcAddr = hdr_eg.ethernet_hdr.dstAddr;
		hdr_eg.ethernet_hdr.dstAddr = tmp_mac;
	}
	action reverse_port(){
		bit<16> tmp_port;
		tmp_port = hdr_eg.udp_hdr.srcPort;
		hdr_eg.udp_hdr.srcPort = hdr_eg.udp_hdr.dstPort;
		hdr_eg.udp_hdr.dstPort = tmp_port;
	}
	action update_netcache_warmupreq_inswitch_to_netcache_warmupreq_inswitch_pop_clone_for_pktloss_and_warmupack(bit<10> switchos_sid,bit<16> reflector_port) {
		hdr_eg.op_hdr.optype = NETCACHE_WARMUPREQ_INSWITCH_POP;
		hdr_eg.shadowtype_hdr.shadowtype = NETCACHE_WARMUPREQ_INSWITCH_POP;
		hdr_eg.udp_hdr.dstPort = reflector_port;
		hdr_eg.clone_hdr.clonenum_for_pktloss = 3; // 3 ACKs (drop w/ 3 -> clone w/ 2 -> clone w/ 1 -> clone w/ 0 -> drop and clone for WARMUPACK to client)

		hdr_eg.clone_hdr.setValid(); // NOTE: clone_hdr.server_sid is reset as 0 in process_scanreq_split_tbl and prepare_for_cachepop_tbl
		hdr_eg.frequency_hdr.setInvalid();
		//hdr_eg.standard_metadata.egress_port = hdr_eg.port; // set eport to switchos
	...
	action forward_netcache_warmupreq_inswitch_pop_clone_for_pktloss_and_warmupack(bit<10> switchos_sid) {
		hdr_eg.clone_hdr.clonenum_for_pktloss =hdr_eg.clone_hdr.clonenum_for_pktloss - 1;
	...
	action update_netcache_warmupreq_inswitch_pop_to_warmupack_by_mirroring(bit<10> client_sid,bit<16> server_port) {
		hdr_eg.op_hdr.optype = WARMUPACK;
		...
		hdr_eg.udp_hdr.srcPort = server_port;
		hdr_eg.udp_hdr.dstPort = hdr_eg.clone_hdr.client_udpport;
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.shadowtype_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.clone_hdr.setInvalid();

		mark_to_drop(standard_metadata); // Disable unicast, but enable mirroring
		// clone_egress_pkt_to_egress(client_sid); // clone to client (hdr_eg.inswitch_hdr.client_sid)
		clone(CloneType.E2E, (bit<32>)client_sid);
	}

	action update_getreq_inswitch_to_getreq() {
		hdr_eg.op_hdr.optype = GETREQ;
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.shadowtype_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.clone_hdr.setInvalid();
		//hdr_eg.standard_metadata.egress_port = hdr_eg.eport;
	}
	...
	action update_getreq_inswitch_to_netcache_getreq_pop_clone_for_pktloss_and_getreq(bit<10> switchos_sid,bit<16> reflector_port) {
		hdr_eg.op_hdr.optype = NETCACHE_GETREQ_POP;
		hdr_eg.udp_hdr.dstPort = reflector_port;
		hdr_eg.clone_hdr.clonenum_for_pktloss = 3; // 3 ACKs (drop w/ 3 -> clone w/ 2 -> clone w/ 1 -> clone w/ 0 -> drop and clone for GETREQ to server)
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.shadowtype_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.clone_hdr.setValid(); // NOTE: clone_hdr.server_sid has been set in prepare_for_cachepop_tbl
	...
	action update_netcache_getreq_pop_to_getreq_by_mirroring(bit<10> server_sid) {
		...
		hdr_eg.udp_hdr.dstPort = hdr_eg.clone_hdr.server_udpport;

		hdr_eg.clone_hdr.setInvalid();
		hdr_eg.frequency_hdr.setInvalid();
		mark_to_drop(standard_metadata); // Disable unicast, but enable mirroring
		// clone_egress_pkt_to_egress(server_sid); // clone to client (hdr_eg.inswitch_hdr.client_sid)
	...
	action update_getreq_inswitch_to_getres_by_mirroring(bit<10> client_sid,bit<16> server_port,bit<8> stat) {
		hdr_eg.op_hdr.optype = GETRES;
		hdr_eg.shadowtype_hdr.shadowtype = GETRES;
		hdr_eg.stat_hdr.stat = stat;
		hdr_eg.stat_hdr.nodeidx_foreval = SWITCHIDX_FOREVAL;
	...
	action update_cache_pop_inswitch_to_cache_pop_inswitch_ack_drop_and_clone(bit<10>switchos_sid,bit<16> reflector_port) {
		...
		hdr_eg.shadowtype_hdr.setInvalid();
		hdr_eg.seq_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.stat_hdr.setInvalid();
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.clone_hdr.setInvalid();

		hdr_eg.op_hdr.optype = CACHE_POP_INSWITCH_ACK;
		hdr_eg.udp_hdr.dstPort = reflector_port;
		mark_to_drop(standard_metadata); // Disable unicast, but enable mirroring
		// clone_egress_pkt_to_egress(switchos_sid); // clone to switchos
		clone(CloneType.E2E, (bit<32>)switchos_sid);
	...
	action update_putreq_inswitch_to_putreq_seq() {
		hdr_eg.op_hdr.optype = PUTREQ_SEQ;
		hdr_eg.shadowtype_hdr.shadowtype = PUTREQ_SEQ;
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.seq_hdr.setValid();
		hdr_eg.clone_hdr.setInvalid();
		//hdr_eg.standard_metadata.egress_port = hdr_eg.eport;
	}

	action update_putreq_inswitch_to_netcache_putreq_seq_cached() {
		hdr_eg.op_hdr.optype = NETCACHE_PUTREQ_SEQ_CACHED;
		hdr_eg.shadowtype_hdr.shadowtype = NETCACHE_PUTREQ_SEQ_CACHED;
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.seq_hdr.setValid();
		hdr_eg.clone_hdr.setInvalid();
	...
	action update_delreq_inswitch_to_delreq_seq() {
		hdr_eg.op_hdr.optype = DELREQ_SEQ;
		hdr_eg.shadowtype_hdr.shadowtype = DELREQ_SEQ;
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.seq_hdr.setValid();
		hdr_eg.clone_hdr.setInvalid();
	...
	action update_delreq_inswitch_to_netcache_delreq_seq_cached() {
		hdr_eg.op_hdr.optype = NETCACHE_DELREQ_SEQ_CACHED;
		hdr_eg.shadowtype_hdr.shadowtype = NETCACHE_DELREQ_SEQ_CACHED;
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.seq_hdr.setValid();
		hdr_eg.clone_hdr.setInvalid();
	...
	action update_cache_evict_loadfreq_inswitch_to_cache_evict_loadfreq_inswitch_ack_drop_and_clone(bit<10> switchos_sid,bit<16> reflector_port) {
		hdr_eg.op_hdr.optype = CACHE_EVICT_LOADFREQ_INSWITCH_ACK;
		hdr_eg.udp_hdr.dstPort = reflector_port;

		hdr_eg.shadowtype_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.frequency_hdr.setValid();
		hdr_eg.clone_hdr.setInvalid();
		mark_to_drop(standard_metadata); // Disable unicast, but enable mirroring
		// clone_egress_pkt_to_egress(switchos_sid); // clone to switchos
		clone(CloneType.E2E, (bit<32>)switchos_sid);
	...
	action update_netcache_valueupdate_inswitch_to_netcache_valueupdate_ack() {
		hdr_eg.op_hdr.optype = NETCACHE_VALUEUPDATE_ACK;
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.shadowtype_hdr.setInvalid();
		hdr_eg.seq_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
	...
	action update_putreq_largevalue_inswitch_to_putreq_largevalue_seq() {
		...
		hdr_eg.op_hdr.optype = PUTREQ_LARGEVALUE_SEQ;
		hdr_eg.shadowtype_hdr.shadowtype = PUTREQ_LARGEVALUE_SEQ;
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.seq_hdr.setValid();
		hdr_eg.clone_hdr.setInvalid();
	...
	action update_putreq_largevalue_inswitch_to_putreq_largevalue_seq_cached() {
		...
		hdr_eg.op_hdr.optype = PUTREQ_LARGEVALUE_SEQ_CACHED;
		hdr_eg.shadowtype_hdr.shadowtype = PUTREQ_LARGEVALUE_SEQ_CACHED;
		hdr_eg.frequency_hdr.setInvalid();
		hdr_eg.inswitch_hdr.setInvalid();
		hdr_eg.seq_hdr.setValid();
		hdr_eg.clone_hdr.setInvalid();
	...
	action update_ipmac_srcport_server2client(bit<48> client_mac,bit<48> server_mac, bit<32> client_ip,bit<32> server_ip, bit<16> server_port) {
		bit <48> tmp_mac;
		bit <32> tmp_ip;
		// hdr_eg.ethernet_hdr.srcAddr = server_mac;
		// hdr_eg.ethernet_hdr.dstAddr = client_mac;
		// hdr_eg.ipv4_hdr.srcAddr = server_ip;
		// hdr_eg.ipv4_hdr.dstAddr = client_ip;
		tmp_mac = hdr_eg.ethernet_hdr.srcAddr;
		hdr_eg.ethernet_hdr.srcAddr = hdr_eg.ethernet_hdr.dstAddr;
		hdr_eg.ethernet_hdr.dstAddr = tmp_mac;
		tmp_ip = hdr_eg.ipv4_hdr.srcAddr;
	...
	action update_ipmac_srcport_switch2switchos(bit<48> client_mac,bit<48> switch_mac, bit<32> client_ip,bit<32> switch_ip, bit<16> client_port) {
		hdr_eg.ethernet_hdr.srcAddr = client_mac;
		hdr_eg.ethernet_hdr.dstAddr = switch_mac;
		hdr_eg.ipv4_hdr.srcAddr = client_ip;
		hdr_eg.ipv4_hdr.dstAddr = switch_ip;
		hdr_eg.udp_hdr.srcPort = client_port;
	}
	...
	action update_dstipmac_client2server(bit<48> server_mac, bit<32> server_ip) {
		hdr_eg.ethernet_hdr.dstAddr = server_mac;
		hdr_eg.ipv4_hdr.dstAddr = server_ip;
	}
	...
	action update_pktlen(bit<16> udplen,bit<16> iplen) {
		hdr_eg.udp_hdr.hdrlen = udplen;
		hdr_eg.ipv4_hdr.totalLen = iplen;
	}

	action add_pktlen(bit<16> udplen_delta,bit<16>  iplen_delta) {
		hdr_eg.udp_hdr.hdrlen = hdr_eg.udp_hdr.hdrlen  + udplen_delta;
		hdr_eg.ipv4_hdr.totalLen = hdr_eg.ipv4_hdr.totalLen + iplen_delta;
	}
	...
	apply{
		...
		// if(meta.spine_sid == 0 && hdr_eg.op_hdr.optype == CACHE_POP_INSWITCH){}
		...
		update_pktlen_tbl.apply(); // Update udl_hdr.hdr_egLen for pkt with variable-length value
