...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    action cs_hash(in bit<32> ipv4_addr, out bit<32> h1, out bit<32> h2, out bit<32> h3, out bit<32> h4) {
        // ddosd upstream uses a patched v1model/p4c that adds custom HashAlgorithm tags
        // (h1..h4 / g1..g4). For portability in our toolchain, use standard crc32 with
        // distinct base seeds to model multiple independent hash functions.
        //
        // For bug finding / regression specs, we also support a deterministic "hash"
        // mode to avoid excessive nondeterminism in the verifier.
#ifdef PROCURATOR_DETERMINISTIC_HASH
        bit<32> idx = (bit<32>)(ipv4_addr & 32w1);
        h1 = idx;
        h2 = idx;
        h3 = idx;
        h4 = idx;
#else
        // Hash outputs must stay within the sketch width.
        hash(h1, HashAlgorithm.crc32, 32w0, {ipv4_addr}, 32w975);
        hash(h2, HashAlgorithm.crc32, 32w1, {ipv4_addr}, 32w975);
        hash(h3, HashAlgorithm.crc32, 32w2, {ipv4_addr}, 32w975);
        hash(h4, HashAlgorithm.crc32, 32w3, {ipv4_addr}, 32w975);
#endif
    }

    action cs_ghash(in bit<32> ipv4_addr, out int<32> g1, out int<32> g2, out int<32> g3, out int<32> g4) {
        // ghash is a 1-bit hash (0/1) used to generate the CountSketch +/- signs.
#ifdef PROCURATOR_DETERMINISTIC_HASH
        g1 = 1;
        g2 = 1;
        g3 = 1;
        g4 = 1;
#else
        hash(g1, HashAlgorithm.crc32, 32w100, {ipv4_addr}, 32w1);
        hash(g2, HashAlgorithm.crc32, 32w101, {ipv4_addr}, 32w1);
        hash(g3, HashAlgorithm.crc32, 32w102, {ipv4_addr}, 32w1);
        hash(g4, HashAlgorithm.crc32, 32w103, {ipv4_addr}, 32w1);
#endif
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            if (meta.pkt_num != m) {
                ...
                meta.src_entropy = ((bit<32>)log2_m_aux << 4) - (src_S_aux >> (bit<32>)log2_m_aux);
                meta.dst_entropy = ((bit<32>)log2_m_aux << 4) - (dst_S_aux >> (bit<32>)log2_m_aux);
