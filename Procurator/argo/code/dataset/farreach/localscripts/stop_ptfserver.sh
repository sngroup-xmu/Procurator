...
then
	...
	do
		...
		then
			kill -15 ${ptf_popserverpids[i]}
		fi
	done
fi

ptf_snapshotserverpids=( "$(ps -aux | grep "ptf_snapshotserver" | grep -v "grep" | awk '{print $2}')" )

if [ ${#ptf_snapshotserverpids[@]} -gt 0 ]
then
	for i in ${!ptf_snapshotserverpids[@]}
	do
		if [ "${ptf_snapshotserverpids[i]}x" != "x" ]
		then
			kill -15 ${ptf_snapshotserverpids[i]}
		fi
	done
fi
