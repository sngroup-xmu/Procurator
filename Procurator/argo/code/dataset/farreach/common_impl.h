...
#define CURMETHOD_ID FARREACH_ID
...
typedef GetResponseSeq<netreach_key_t, val_t> get_response_seq_t;
//typedef PutResponse<netreach_key_t> put_response_t;
typedef PutResponseSeq<netreach_key_t> put_response_seq_t;
//typedef DelResponse<netreach_key_t> del_response_t;
typedef DelResponseSeq<netreach_key_t> del_response_seq_t;
...
typedef GetResponseLargevalueSeq<netreach_key_t, val_t> get_response_largevalue_seq_t;
...
short client_upstreambackupreleaser_port = 0;
...
inline void parse_ini(const char* config_file) {
	...
	client_upstreambackupreleaser_port = ini.get_client_upstreambackupreleaser_port();
	...
	COUT_VAR(client_upstreambackupreleaser_port);
	printf("\n");
