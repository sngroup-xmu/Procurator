...
control Cache(inout headers hdr,
              ...
              inout ingress_intrinsic_metadata_for_deparser_t ig_intr_dprsr_md)
{
    ...
    RegisterAction<timestamp_t, switch_id_t, bool>(bf) check_timeout = {  
        void apply(inout timestamp_t val, out bool rv) {
            if (ig_intr_prsr_md.global_tstamp[47:16] > val) {
                val = ig_intr_prsr_md.global_tstamp[47:16] + 0xffff; //+ retransmit_timeout.read();
                rv = true;
            } else {
                rv = false;
            }
    ...
    apply {
            index_t idx = dst_index_hash.get({ig_md.switchv2p_md.key}); // V TODO:
        if (!ig_md.switchv2p_md.to_gw && ig_md.switchv2p_md.switch_type == TOR && 
            (hdr.switchv2p.type == NORMAL_TYPE || hdr.switchv2p.type == EVICTION_TAG_TYPE)) {
                idx = src_index_hash.get({hdr.ipv4_inner.src_addr}); // V TODO:
            ig_md.switchv2p_md.key = hdr.ipv4_inner.src_addr;
            ig_md.switchv2p_md.val = hdr.ipv4_outter.src_addr;
        }
        
        check_misdelivered.apply();
        // 论文里说的是：learning包，只给发送端的 tor（中间经过的交换机，不应该学吧？）
        if (hdr.switchv2p.type == LEARNING_TYPE) {
            if (ig_md.switchv2p_md.switch_type == TOR) {
                set_key.execute(idx);
                set_val.execute(idx);
            }
            ...
            if (ig_md.switchv2p_md.switch_type == TOR && ig_md.switchv2p_md.misdelivered) {
                    // V TODO:
                ...
                if (in_cache) {
                    if (ig_md.switchv2p_md.switch_type == SPINE) {
                        ipv4_addr_t val = get_val.execute(idx);
                        add_eviction_tag(val); // promotion
                    } else {
        ...
        } else if (ig_md.switchv2p_md.switch_type == TOR) {
            set_key.execute(idx);
            set_val.execute(idx);
        } else if ((ig_md.switchv2p_md.switch_type == CORE && hdr.switchv2p.type == EVICTION_TAG_TYPE) ||
                   (ig_md.switchv2p_md.switch_type == SPINE || ig_md.switchv2p_md.switch_type == GW_SPINE)) {
            ipv4_addr_t prev_ipv4 = set_key_if_not_active.execute(idx);
            // 只有当这个 entry 没人用、或者用了但是没hit过（not active）
            if (prev_ipv4 != 0) {
                hdr.switchv2p.key = prev_ipv4;
                hdr.switchv2p.val = set_val.execute(idx);
                if (hdr.switchv2p.key == 1) { // 默认值，说明这个 entry 没人用
                    hdr.switchv2p.type = NORMAL_TYPE;
                } else { // 否则，就是覆盖了一个cache了但是没有hit过的entry；同样要尽可能移到别的交换机
                    hdr.switchv2p.type = EVICTION_TAG_TYPE;
                }
            }
        } else if (ig_md.switchv2p_md.switch_type == GW_TOR) {
            // V TODO:
            ...
            hdr.switchv2p.key = set_key.execute(idx); // 会返回旧的 key
            hdr.switchv2p.val = set_val.execute(idx);
            if (hdr.switchv2p.key == 1) { // 默认值，说明这个 entry 没人用
                hdr.switchv2p.type = NORMAL_TYPE;
            } else { // 否则，就是覆盖了旧的cache entry；把原来的mapping，跟着包走，尽可能移到别的交换机去
                hdr.switchv2p.type = EVICTION_TAG_TYPE;
            }
        }
    }
