...
#include "verify/channel_extract/chan_extractor.h"
#include "verify/to_dsl/field_mapping.h"
...
private:
	...
	cstring declaration;
	cstring code;
	std::ostream& out_typedef; // for typedef code
	std::ostream& out; // for other code except typedef
	int indent = 0;
	...
	std::unordered_map<cstring, int> defList;
	// for header stack
	std::map<cstring, const IR::ParserState*> parserStates;
	std::map<cstring, int> parserStateRenameTimes;
	std::map<cstring, int> stackSize;
	const int MAX_INLINE_DEPTH = 8;

	std::unordered_map<cstring, const IR::Declaration_Instance*> instances;
	std::set<cstring> stacks;
	std::set<cstring> functions;
	// dependency, pred[proc] store procedures that call procedure 'proc'
	std::map<cstring, std::set<cstring>> pred; 
	...
	std::unordered_map<cstring, int> enumTypeMap;

	std::set<cstring> promela_keyword = {"priority", "len"};

	ChanExtractor header_extractor;
	FieldMapping field_mapping; // p4->promela
...
public:
	Translator(std::ostream &out_typedef, std::ostream &out, P4VerifyOptions &options, BMV2CmdsAnalyzer* bMV2CmdsAnalyzer = nullptr);
	void writeToFile();
	...
	// Statement
	cstring translate(const IR::Statement *stat, const bool firstIfStat=true);
	...
	cstring translate(const IR::IfStatement *ifStatement, const bool firstIfStat);
	...
	cstring translate(const IR::StructExpression* structExpression);
	cstring translate(const IR::NamedExpression* namedExpression);
	...
	cstring translate(const IR::Type_Method *method);
	...
	cstring stringWithPrefix(const cstring& str);
	cstring removePrefix(const cstring& str);
	cstring handle_keyword(cstring id);
	void generate_struct_assign_method(cstring typeName, const IR::IndexedVector<IR::StructField> fields);
	void generate_struct_reset_method(cstring typeName, const IR::IndexedVector<IR::StructField> fields);
};
