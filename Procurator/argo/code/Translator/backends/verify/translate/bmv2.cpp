...
bool BMV2CmdsAnalyzer::hasTableSetDefaultCmd(cstring table){
	for(auto cmd:cmds){
...
TableSetDefault* BMV2CmdsAnalyzer::getTableSetCmd(cstring table){
	for(auto cmd:cmds){
		if(cmd->cmdType == NAME_TABLE_SET_DEFAULT){
			TableSetDefault* tableSet = (TableSetDefault*)cmd;
			if(isSame(table, tableSet->table)){
...
cstring TableAdd::getCondition(std::vector<cstring> keys) {
	...
	for (int i = 0; i < fields.size(); ++i) {
		...
		if (field_value.find("/")) { // lpm
			std::string::size_type idx = ((std::string)field_value.c_str()).find("/");
			cstring ip = str2num(field_value.substr(0, idx));
			int bits = std::stoi(field_value.substr(idx+1).c_str());
			condition += key;
			if (bits < 32) {
				int mask = ((1 << bits) - 1) << (32 - bits);
				condition += " & " + cstring::to_cstring(mask);
			}
			condition += " == " + ip;
		} else if (field_value.find("&&&")) { // ternary, 
			std::string::size_type idx = ((std::string)field_value.c_str()).find("&&&");
			cstring value = str2num(field_value.substr(0, idx));
			cstring mask = str2num(field_value.substr(idx));
			condition += "(" + key + " & " + mask + ") == (" + value + " & " + mask + ")";
		} else if (field_value.find("->")){ // range
			std::string::size_type idx = ((std::string)field_value.c_str()).find("->");
			auto min = str2num(field_value.substr(0, idx));
			auto max = str2num(field_value.substr(idx+2));
			condition += "(" + key + " >= " + min + ") && (" + key + " <= " + max + ")";
		} else { // exact
			condition += key + " == " + str2num(field_value);
		}
	}
...
cstring TableAdd::getParas() {
	cstring paras = "";
	for (auto para : parameters) {
		cstring val = str2num(para);
		if (paras != "") {
			paras += ", ";
		}
		paras += val;
