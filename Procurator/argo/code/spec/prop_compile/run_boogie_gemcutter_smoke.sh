...
#
# Usage (example):
#   Procurator/argo/code/spec/prop_compile/run_boogie_gemcutter_smoke.sh \
...
SPEC="$REPO_ROOT/Procurator/argo/code/spec/test/boogie_smoke.prop"
...
TOOLCHAIN="$REPO_ROOT/Procurator/argo/code/spec/config/ClosureCheck-ReachSafety.xml"
SETTINGS="$REPO_ROOT/Procurator/argo/code/spec/config/ReachSafety-32bit-GemCutter-ALL.epf"
...
if [[ -z "$P4B_BIN" ]]; then
  if [[ -x "$REPO_ROOT/P4B-Translator/build-host/p4c-translator" ]]; then
    P4B_BIN="$REPO_ROOT/P4B-Translator/build-host/p4c-translator"
  else
    echo "[ERR] missing --p4b-bin (and default $REPO_ROOT/P4B-Translator/build-host/p4c-translator not found)" >&2
    exit 2
  fi
fi
