	...
	public static TermVarsFuns computeTermVarsProc(final Term term, final ManagedScript mgdScript,
		...
		if (nonTheoryAppTerms.isEmpty()) {
			...
			for (final ApplicationTerm nonTheoryAppTerm : nonTheoryAppTerms) {
				IProgramFunction progFunc = funcSymb2ProgFunc.apply(nonTheoryAppTerm.getFunction());
				if (progFunc == null) {
					// Some toolchains introduce uninterpreted functions without registering them in the symbol table.
					// Fall back to a fresh ProgramFunction to keep trace checking robust.
					progFunc = new ProgramFunction(nonTheoryAppTerm.getFunction());
				}
				tmp.add(progFunc);
			}
