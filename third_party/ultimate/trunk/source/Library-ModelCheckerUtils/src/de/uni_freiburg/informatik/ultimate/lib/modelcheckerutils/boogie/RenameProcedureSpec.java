...
/**
 * Rename signature procedures specification n a way that it is compatible to the implementation of the procedure. E.g.
 * we have specification <br>
 * {@code procedure foo(x:int) returns (y:bool);}<br>
 * and an implementation<br>
 * {@code implementation foo(a:int) returns (b:bool)}<br>
 * then this class can be used to obtain a specification where x is renamed to a and y is renamed to b.<br>
...
public class RenameProcedureSpec extends BoogieTransformer {
	...
	public void buildRenaming(final VarList[] specVars, final VarList[] implVars) {
		...
		for (final VarList specVar : specVars) {
			final String[] specIds = specVar.getIdentifiers();
			for (final String specId : specIds) {
				...
				assert specVar.getType().getBoogieType().equals(implVars[j1 - 1].getType().getBoogieType());
				if (!specId.equals(implIds[j2])) {
					mRenaming.put(specId, implIds[j2]);
				}
				j2++;
			}
		}
	...
	public Expression processExpression(final Expression expr) {
		...
		if (expr instanceof IdentifierExpression) {
			...
			if (newName != null) {
				final IdentifierExpression newExpr = new IdentifierExpression(expr.getLocation(), expr.getType(),
						newName, id.getDeclarationInformation());
