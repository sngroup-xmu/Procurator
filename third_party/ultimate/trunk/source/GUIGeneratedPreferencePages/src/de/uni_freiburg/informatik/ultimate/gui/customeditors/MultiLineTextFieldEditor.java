...
/**
 * MultiLineTextFieldEditor. Field editor that is same as string field editor but will have the multi line text field
 * for user input.
 *
...
public final class MultiLineTextFieldEditor extends FieldEditor {
	 ...
	 * <p>
	 * Subclasses must implement this method to adjust the horizontal span of controls so they appear correct in the
	...
	public Text getTextControl(final Composite parent) {
		if (mTextField == null) {
			mTextField = new Text(parent, SWT.MULTI | SWT.V_SCROLL | SWT.BORDER | SWT.WRAP);
	 ...
	 * <p>
	 * The default implementation of this framework method returns <code>true</code>. Subclasses wishing to perform
	 * validation should override both this method and <code>refreshValidState</code>.
	 * </p>
	 *
	 * @return <code>true</code> if the field value is valid, and <code>false</code> if invalid
	...
	/**
	 * Refreshes this field editor's valid state after a value change and fires an <code>IS_VALID</code> property change
	 * event if warranted.
	 * <p>
	 * The default implementation of this framework method does nothing. Subclasses wishing to perform validation should
	 * override both this method and <code>isValid</code>.
	 * </p>
	 */
	@Override
