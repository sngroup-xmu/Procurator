...
public class SymbolTableTransformer extends BoogieTransformer {
	...
	public SymbolTableTransformer(final ILogger logger) {
		mLogger = logger;
		...
		if (decl instanceof Procedure) {
			...
			for (final VarList vl : vdecl.getVariables()) {
				for (final String varName : vl.getIdentifiers()) {
					mSymbolTable.addGlobalVariable(varName, vl.getType().getBoogieType());
				}
			}
		}
	...
	private void addVariables(final VariableDeclaration[] vdecls) {
		for (final VariableDeclaration vdecl : vdecls) {
			addVariables(vdecl.getVariables());
		}
	}

	private void addVariables(final VarList[] vlists) {
		for (final VarList vl : vlists) {
			for (final String varName : vl.getIdentifiers()) {
				mSymbolTable.addLocalVariable(varName, mCurrentScopeIdentifier, vl.getType().getBoogieType());
			}
		}
	}
