...
/**
 *
 * ObserverDispatcher provides a flexible way to combine a certain way of graph traversal (in,pre,postorder,
 * breadth-first) with a certain order of observer execution (either for each observer a own traversal, or in each node
 * all observers are called one after another).
 *
...
public abstract class ObserverDispatcher {
	...
	public ObserverDispatcher(final ILogger logger) {
		mObservers = new LinkedList<>();
		mLogger = logger;
	}

	public void setWalker(final IRCFGWalker walker) {
		mWalker = walker;
	}
