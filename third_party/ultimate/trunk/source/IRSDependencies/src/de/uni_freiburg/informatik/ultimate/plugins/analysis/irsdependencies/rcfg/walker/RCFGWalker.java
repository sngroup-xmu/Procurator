...
/**
 * The abstract class RCFGWalker provides standard observer handling (add, remove, removeAll, contains) for all
 * traversals of RCFG graphs. Basic usage is as follows:
 *
 ...
 * Call "run" on RCFG.
 *
 * As walkers are responsible for the traversal, they normally contain the algorithm necessary for searches and
 * heuristics. The actual work is then done by observers.
 *
 * Please note that if you do not attach a observer to a walker, the dispatcher will prevent executing the walking code.
 *
 * @author dietsch
 *
 */
public abstract class RCFGWalker implements IRCFGWalker {
	...
	public RCFGWalker(final ObserverDispatcher dispatcher, final ILogger logger) {
		mDispatcher = dispatcher;
		mLogger = logger;
	}

	@Override
	public boolean addObserver(final IObserver observer) {
		return mDispatcher.addObserver(observer);
	}

	@Override
	public boolean removeObserver(final IObserver observer) {
	...
	@Override
	public boolean containsObserver(final IObserver observer) {
	...
	protected void programExitReached() {
		mDispatcher.callObservers(SimpleRCFGVisitor::endOfTrace);
	}

	protected void pre(final IcfgLocation node) {
		mDispatcher.callObservers(visitor -> visitor.pre(node));
	}

	protected void pre(final IcfgEdge edge) {
		mDispatcher.callObservers(visitor -> visitor.pre(edge));
	}

	protected void post(final IcfgLocation node) {
		mDispatcher.callObservers(visitor -> visitor.post(node));
	}

	protected void post(final IcfgEdge edge) {
		mDispatcher.callObservers(visitor -> visitor.post(edge));
	}

	protected void level(final IcfgLocation node) {
		mDispatcher.callObservers(visitor -> visitor.level(node));
	}

	protected void level(final IcfgEdge edge) {
		mDispatcher.callObservers(visitor -> visitor.level(edge));
	}
}
