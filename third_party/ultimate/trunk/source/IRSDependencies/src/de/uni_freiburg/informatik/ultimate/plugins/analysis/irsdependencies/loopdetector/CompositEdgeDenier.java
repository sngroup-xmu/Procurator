...
public final class CompositEdgeDenier<E> implements IEdgeDenier<E> {
	...
	public CompositEdgeDenier(final Collection<IEdgeDenier<E>> edgeDenier) {
		if (edgeDenier != null) {
			mEdgeDeniers = new ArrayList<>(edgeDenier.size());
			mEdgeDeniers.addAll(edgeDenier);
		} else {
	...
	private static final class CachedIterator<E> implements Iterator<E> {
		...
		private CachedIterator(final Iterator<E> iter) {
			mIter = iter;
			mUsed = false;
			mComplete = false;
