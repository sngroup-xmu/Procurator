...
public class SymbolTable {
	...
	void addLocalVariable(final String identifier, final String procedure, final IBoogieType type) {
		if (procedure.equals(sGlobalScopeIdentifier)) {
			throw new IllegalArgumentException(
					"Procedure name is equal to global scope identifier " + sGlobalScopeIdentifier);
		}
		final CompleteBoogieVar var = new CompleteBoogieVar(identifier, procedure, type);
	...
	void addGlobalVariable(final String identifier, final IBoogieType type) {
	...
	public CompleteBoogieVar getVariable(final String variableName, final String procedureName) {
		HashMap<String, CompleteBoogieVar> scope;
	...
	public List<CompleteBoogieVar> getVariables(final String procedureName) {
		final List<CompleteBoogieVar> rtrList = new LinkedList<>();
		HashMap<String, CompleteBoogieVar> scope;
		if (mScopes.containsKey(sGlobalScopeIdentifier)) {
			scope = mScopes.get(sGlobalScopeIdentifier);
		...
		if ((procedureName != null) && mScopes.containsKey(procedureName)) {
			scope = mScopes.get(procedureName);
	...
	public String toString() {
		...
		sb.append("\n");
		for (final Entry<String, HashMap<String, CompleteBoogieVar>> entry : mScopes.entrySet()) {
			...
			for (final Entry<String, CompleteBoogieVar> innerEntry : entry.getValue().entrySet()) {
				sb.append("Name is \"");
