...
public abstract class IRSDependenciesAnnotation extends AbstractAnnotations {
	...
	public static <T extends IRSDependenciesAnnotation> T getAnnotation(final IElement e, final Class<?> c) {
		if (e.hasPayload()) {
			final Object rtr = e.getPayload().getAnnotations().get(Activator.PLUGIN_ID + c.getName());
			if (rtr != null) {
				return (T) rtr;
			}
		}
	...
	public void addAnnotation(final IElement e) {
		e.getPayload().getAnnotations().put(Activator.PLUGIN_ID + this.getClass().getName(), this);
	}
