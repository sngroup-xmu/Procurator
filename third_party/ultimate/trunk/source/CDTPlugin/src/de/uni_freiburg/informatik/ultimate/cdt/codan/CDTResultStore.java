...
/**
 * Static class which stores the Ultimate Results per File. It is accessible for all parts of the CDT-Plugin, especially
 * for the Views.
 *
...
public class CDTResultStore {
	 ...
	 *            list of results
	 */
	public static void addResults(final String file, final String tool, final List<IResult> value) {
		if (fileToResults == null) {
			fileToResults = new HashMap<>();
		}
		if (!fileToResults.containsKey(file)) {
			fileToResults.put(file, new HashMap<>());
		}
		fileToResults.get(file).put(tool, value);
	}

	public static int addHackyResult(final IResult result) {
	...
	public static IResult getHackyResult(final int i) {
		if (hackyResultList == null) {
			hackyResultList = new ArrayList<>();
		}
	 ...
	 */
	public static List<IResult> getResults(final String key) {
		if (fileToResults != null && fileToResults.containsKey(key)) {
			final ArrayList<IResult> compResults = new ArrayList<>();
