...
public class UltimatePerspective implements IPerspectiveFactory {
	...
	public void createInitialLayout(final IPageLayout layout) {
		...
		// editor area.
		final IFolderLayout left = layout.createFolder("left", IPageLayout.LEFT, (float) 0.2, editorArea);
		left.addView(IPageLayout.ID_PROJECT_EXPLORER);

		// Here we declare what we place right of the editor area
		// First we use right1 to place the ResultList
		final IFolderLayout right1 = layout.createFolder("right1", IPageLayout.RIGHT, (float) 0.7, editorArea);
		right1.addView(ResultList.ID);
		// Then we define right2 to place the LocationTrace
		final IFolderLayout right2 = layout.createFolder("right2", IPageLayout.BOTTOM, (float) 0.2, "right1");
		right2.addView(LocationTrace.ID);
		// Now we add the VariableAssignment View under the LocationTrace
		final IFolderLayout right3 = layout.createFolder("right3", IPageLayout.BOTTOM, (float) 0.5, "right2");
		right3.addView(VariableAssignment.ID);

		// Here is what we place under the editor area
		final IFolderLayout bottom = layout.createFolder("bottom", IPageLayout.BOTTOM, (float) 0.7, editorArea);
