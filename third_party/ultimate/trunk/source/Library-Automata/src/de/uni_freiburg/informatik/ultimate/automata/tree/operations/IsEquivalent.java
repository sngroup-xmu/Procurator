...
/**
 * Computes if the languages of two given bottom-up tree automata are equivalent.
 *
 * This means that the result is <tt>true</tt> if all accepted runs of the of one automaton are also accepted by the
 * other automaton and vice versa, else it is <tt>false</tt>.<br>
 *
 * If the equivalence does not hold then {@link #getCounterexample()} offers such a tree run that is not accepted by one
 * automaton while accepted by the other.
 *
...
public final class IsEquivalent<LETTER extends IRankedLetter, STATE>
	...
	/**
	 * If the equivalence does not hold then this field holds a tree run that acts as counterexample to the equivalence.
	 * This means it represents a run that is accepted by one of the operands but not by the other.
	 */
	...
	/**
	 * The result of the equivalence check, <tt>true</tt> if the languages of both operands are equivalent,
	...
	/**
	 * Computes if the languages of two given bottom-up tree automata are equivalent.<br>
	 * <br>
	 * This means that the result is <tt>true</tt> if all accepted runs of the of one automaton are also accepted by the
	 * other automaton and vice versa, else it is <tt>false</tt>.<br>
	 * <br>
	 * If the equivalence does not hold then {@link #getCounterexample()} offers such a tree run that is not accepted by
	 * one automaton while accepted by the other.
	 *
	 * @param <SF>
	 *            The type of the state factory to use for intermediate methods, must provide methods for merging
	 *            states, creating sink states and intersecting states.
	 * @param services
	 *            Ultimate services
	 * @param factory
	...
	public <SF extends IMergeStateFactory<STATE> & ISinkStateFactory<STATE> & IIntersectionStateFactory<STATE>> IsEquivalent(
		...
		mResult = checkEquivalence(factory);
	...
	public String exitMessage() {
		assert mExitMessage != null : "Unknown problem with exit message.";
		return mExitMessage;
	}

	/**
	 * If the equivalence check does not hold then this method offers a tree run that acts as counterexample. This means
	 * that this tree run is accepted by one of the two automata while not by the other. If the equivalence check holds
	 * than this method does not provide such a tree run.
	 *
	 * @return A tree run that acts as counterexample for the equivalence, if present
	 */
	public Optional<TreeRun<LETTER, STATE>> getCounterexample() {
		...
		return Optional.of(mCounterexample);
	}
	...
	/**
	 * Checks if the two given tree automata are equivalent. Therefore inclusion in both direction is computed. If the
	 * equivalence does not hold then {@link #getCounterexample()} offers such a tree run that is not accepted by one
	 * automaton while accepted by the other.
	 *
	 * @param <SF>
	 *            The type of the state factory to use for intermediate methods, must provide methods for merging
	 ...
	 * @param factory
	 *            The state factory to use for intermediate methods.
	 * @return <tt>True</tt> if both automata are equivalent, <tt>false</tt> otherwise
	 * @throws AutomataOperationCanceledException
	 *             If the operation was canceled, for example from the Ultimate framework.
	 */
	private <SF extends IMergeStateFactory<STATE> & ISinkStateFactory<STATE> & IIntersectionStateFactory<STATE>> boolean
			checkEquivalence(final SF factory) throws AutomataOperationCanceledException {
		...
		if (mServices.getProgressAwareTimer() != null && isCancellationRequested()) {
			mLogger.debug("Stopped after isIncluded(first, second).");
	...
	/**
	 * Checks language inclusion in one direction, whether the language of the first operand is included in the language
	 * of the second operand. If not then it also sets a counterexample to the internal fields.
	 *
	 * @param <SF>
	 *            The type of the state factory to use for intermediate methods, must provide methods for merging
	 *            states, creating sink states and intersecting states.
	 ...
	 * @return <tt>True</tt> if the language of the first automaton is included in the language of the second automaton,
	 *         <tt>false</tt> otherwise
	 * @throws AutomataOperationCanceledException
	 *             If the operation was canceled, for example from the Ultimate framework.
	 */
	private <SF extends IMergeStateFactory<STATE> & ISinkStateFactory<STATE> & IIntersectionStateFactory<STATE>> boolean
			checkInclusion(final SF factory, final ITreeAutomatonBU<LETTER, STATE> firstOperand,
					final ITreeAutomatonBU<LETTER, STATE> secondOperand) throws AutomataOperationCanceledException {
		final IsIncluded<LETTER, STATE> isIncluded = new IsIncluded<>(mServices, factory, firstOperand, secondOperand);
		if (!isIncluded.getResult().booleanValue()) {
			mCounterexample = isIncluded.getCounterexample().get();
			return false;
		}
		return true;
	}
	...
	public static void main(final String[] args) throws AutomataOperationCanceledException {
		...
		final ITreeAutomatonBU<StringRankedLetter, String> fourthTree =
				new Minimize<>(services, factory, firstTree).getResult();
		...
		final IsEquivalent<StringRankedLetter, String> isEquivalent =
				new IsEquivalent<>(services, factory, firstTree, fourthTree);
