...
/**
 * Takes a {@link INwaSuccessorStateProvider} and acts as a {@link NwaOutgoingLetterAndTransitionAdapter}. Motivation:
 * {@link INwaSuccessorStateProvider} should only be used to defne successors. This class does the caching (avoid that
 * transitions are constructed twice) and the conversion from outgoing successors to outgoing transitions.
 *
 ...
 */
public class NwaOutgoingLetterAndTransitionAdapter<LETTER, STATE>
	...
	public NwaOutgoingLetterAndTransitionAdapter(
			final INwaSuccessorStateProvider<LETTER, STATE> nwaSuccessorStateProvider) {
	...
	public Iterable<OutgoingInternalTransition<LETTER, STATE>> internalSuccessors(final STATE state,
		...
		return NestedWordAutomataUtils.constructInternalTransitionIteratorFromNestedMap(state, letter,
				mInternalTransitionCache);
	}
	...
	public Iterable<OutgoingCallTransition<LETTER, STATE>> callSuccessors(final STATE state, final LETTER letter) {
		...
		return NestedWordAutomataUtils.constructCallTransitionIteratorFromNestedMap(state, letter,
				mCallTransitionCache);
	}
	...
	public Iterable<OutgoingReturnTransition<LETTER, STATE>> returnSuccessors(final STATE state, final STATE hier,
		...
		if (mReturnTransitionBookkeeping.get(state, hier, letter) == null) {
			final Collection<STATE> computedSuccs =
					mNwaSuccessorStateProvider.returnSuccessorsGivenHier(state, hier, letter);
			for (final STATE succ : computedSuccs) {
				mReturnTransitionCache.put(state, hier, letter, succ, IsContained.IsContained);
			}
		}
		return NestedWordAutomataUtils.constructReturnTransitionIteratorFromNestedMap(state, hier, letter,
				mReturnTransitionCache);
	}
