 ...
 * implementation that constructs automata on-demand (resp. lazily aka on-the-fly). Classes that implement this
 * interface do not have to store all transitions of the automaton explicitly. Instead these classes only have to
 * provide outgoing transitions for a given state or a state/letter combination.
 *
 ...
 */
public interface INwaOutgoingLetterAndTransitionProvider<LETTER, STATE>
		extends INwaOutgoingTransitionProvider<LETTER, STATE> {
	...
	default Iterable<OutgoingInternalTransition<LETTER, STATE>> internalSuccessors(final STATE state) {
		final Function<LETTER, Iterator<OutgoingInternalTransition<LETTER, STATE>>> fun =
				x -> internalSuccessors(state, x).iterator();
		return () -> new NestedIteratorNoopConstruction<>(lettersInternal(state).iterator(), fun);
	}
	...
	@Override
	default Iterable<OutgoingReturnTransition<LETTER, STATE>> returnSuccessorsGivenHier(final STATE state,
			final STATE hier) {
