...
public class BranchingProcessToUltimateModel<LETTER, PLACE> {
	...
	public IElement transformToUltimateModel(final BranchingProcess<LETTER, PLACE> branchingProcess) {
		final BranchingProcessInitialNode<LETTER, PLACE> graphroot =
				new BranchingProcessInitialNode<>(branchingProcess);
		...
		while (!queue.isEmpty()) {
			...
			if (node instanceof Condition) {
				...
				eventHandling(branchingProcess, place2ConditionNode, transition2EventNode, queue,
						(Event<LETTER, PLACE>) node);
			}
		}
		return graphroot;
	}
