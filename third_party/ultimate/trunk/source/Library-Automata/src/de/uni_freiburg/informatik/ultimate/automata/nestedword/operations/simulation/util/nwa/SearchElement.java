...
public final class SearchElement<LETTER, STATE> {
	...
	public SearchElement(final Vertex<LETTER, STATE> vertex, final Vertex<LETTER, STATE> target,
			final Vertex<LETTER, STATE> history, final SummarizeEdge<LETTER, STATE> summarizeEdge,
			final Pair<STATE, Boolean> duplicatorChoice, final Vertex<LETTER, STATE> origin) {
		mVertex = vertex;
		mTarget = target;
		mHistory = history;
	...
	@Override
	public int hashCode() {
		return Objects.hash(mDuplicatorChoice, mHistory, mSummarizeEdge, mTarget, mVertex, mOrigin);
	}
