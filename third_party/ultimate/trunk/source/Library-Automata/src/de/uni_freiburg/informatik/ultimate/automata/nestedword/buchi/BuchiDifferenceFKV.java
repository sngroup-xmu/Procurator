...
public final class BuchiDifferenceFKV<LETTER, STATE> extends AbstractBuchiDifference<LETTER, STATE> {
	...
	private <SF extends IBuchiComplementFkvStateFactory<STATE> & IBuchiIntersectStateFactory<STATE> & IEmptyStackStateFactory<STATE>>
		...
		mSndComplemented = new NwaOutgoingLetterAndTransitionAdapter<>(mOnDemandComplemented);
	...
	public String exitMessage() {
		return "Finished " + getOperationName() + ". First operand " + mFstOperand.sizeInformation()
				...
				+ mOnDemandComplemented.getPowersetStates() + " powerset states" + mOnDemandComplemented.getRankStates()
				+ " rank states. The highest rank that occured is " + mOnDemandComplemented.getHighesRank();
	}
