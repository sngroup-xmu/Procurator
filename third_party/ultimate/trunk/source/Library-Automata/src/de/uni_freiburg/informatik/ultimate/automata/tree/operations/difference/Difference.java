 ...
 * <br>
 * This means that the resulting automaton accepts the language {@code L(firstOperand) \ L(secondOperand)}.
 *
...
public final class Difference<LETTER extends IRankedLetter, STATE>
	 ...
	 * <br>
	 * This means that the resulting automaton accepts the language {@code L(firstOperand) \ L(secondOperand)}.
	 * Therefore the current implementation sees the difference as the equivalent operation
	 * {@code intersect(first, complement(second))}. Thus it falls back to Operations {@link Complement} and
	 * {@link Intersect}.
	 *
	 * @param <SF>
	 *            The type of the state factory to use for intermediate methods, must provide methods for merging
	...
	private <SF extends IMergeStateFactory<STATE> & ISinkStateFactory<STATE> & IIntersectionStateFactory<STATE>>
			ITreeAutomatonBU<LETTER, STATE> computeDifference(final SF factory)
					throws AutomataOperationCanceledException {
		// Difference can be seen as intersect(first, complement(second))
		// First compute complement(second)
		if (mLogger.isDebugEnabled()) {
			mLogger.debug("Starting to compute complement(second).");
		}
		final Collection<LETTER> alphabet = new HashSet<>(mFirstOperand.getAlphabet());
		alphabet.addAll(mSecondOperand.getAlphabet());

		final ITreeAutomatonBU<LETTER, STATE> secondOperandComplemented =
				new Complement<>(mServices, factory, mSecondOperand, alphabet).getResult();

		// If operation was canceled, for example from the
		// Ultimate framework
		if (mServices.getProgressAwareTimer() != null && isCancellationRequested()) {
			mLogger.debug("Stopped after computing complement(second).");
			throw new AutomataOperationCanceledException(this.getClass());
		}

		// Now compute the intersection which is also the final result
		if (mLogger.isDebugEnabled()) {
			mLogger.debug("Starting to compute intersect(first, secondComplemented).");
		}
		return new Intersect<>(mServices, factory, mFirstOperand, secondOperandComplemented).getResult();
	}
