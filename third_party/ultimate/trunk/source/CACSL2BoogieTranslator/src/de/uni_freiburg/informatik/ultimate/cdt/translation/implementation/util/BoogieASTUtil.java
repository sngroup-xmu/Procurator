...
public class BoogieASTUtil {
	 ...
	 *
	 * @param expr
	 *            the Expression to process (It is assumed, that <code>expr</code> is instance of one class in
	...
	public static String getLeftMostId(final Expression expr) {
		assert expr instanceof IdentifierExpression || expr instanceof StructAccessExpression
				|| expr instanceof ArrayAccessExpression;
		Expression e = expr;
		while (e instanceof ArrayAccessExpression || e instanceof StructAccessExpression) {
	...
	/**
	 * Flattens a LeftHandSide into a String Array. I.e. all accessed identifiers are listed.
	 *
	 * @param lhs
	...
	public static String[] getLHSList(final LeftHandSide lhs) {
		LeftHandSide l = lhs;
		final ArrayList<String> lhsList = new ArrayList<>();
		while (l instanceof ArrayLHS || l instanceof StructLHS) {
			if (l instanceof ArrayLHS) {
				l = ((ArrayLHS) l).getArray();
			} else if (l instanceof StructLHS) {
	 ...
	 *
	 * @param e
	 *            the expression to convert. It is assumed, that <code>e</code> is instance of one class in
	 *            <code>{IdentifierExpression, StructAccessExpression, ArrayAccessExpression}</code> .
	 * @return the created LHS object.
	 */
	public static LeftHandSide getLHSforExpression(final Expression e) {
		assert e instanceof IdentifierExpression || e instanceof StructAccessExpression
		...
		if (e instanceof ArrayAccessExpression) {
			...
			return ExpressionFactory.constructNestedArrayLHS(loc, lhs, aae.getIndices());
		}
