...
public class SMTLibExpectedResultFinder<OVERALL_RESULT> extends AbstractExpectedResultFinder<OVERALL_RESULT> {
	...
	public SMTLibExpectedResultFinder(final OVERALL_RESULT unknownResult, final OVERALL_RESULT satResult,
			final OVERALL_RESULT unsatResult) {
	...
	public void findExpectedResult(final UltimateRunDefinition ultimateRunDefinition) {
		...
		if (file == null) {
			...
			mExpectedResultEvaluation =
					"Could not determine expected result; ulitmateRunDefinition of test case " + "contained no file";
