...
public class SMTTheoryStateFactoryAndPredicateHelper {
	...
	private IPredicate getOrConstructPredicate(final Term resTerm, final Set<IProgramVarOrConst> variables) {
	...
	public SMTTheoryState widen(final SMTTheoryState first, final SMTTheoryState second) {
		return disjoinFlat(first, second);// TODO does this make sense?
	}

	public SMTTheoryState conjoin(final SMTTheoryState first, final SMTTheoryState second) {
		if (first.isBottom()) {
			return first;
		}
		if (second.isBottom()) {
	...
	public SMTTheoryState disjoinFlat(final SMTTheoryState first, final SMTTheoryState second) {
		if (first.isBottom()) {
			return second;
		}
		if (second.isBottom()) {
		...
		for (final Term conjunct : conjunctsFirst) {
		...
		for (final Term conjunct : conjunctsSecond) {
	...
	public boolean impliesLiteral(final SMTTheoryState arrayTheoryState, final Term literalTerm) {
		mMgdScript.lock(this);
		mMgdScript.push(this, 1);
		mMgdScript.assertTerm(this, arrayTheoryState.getPredicate().getClosedFormula());
