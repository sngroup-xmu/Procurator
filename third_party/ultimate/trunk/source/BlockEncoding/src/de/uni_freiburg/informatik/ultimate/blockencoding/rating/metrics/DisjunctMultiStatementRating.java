...
/**
 * This metric is basically a mixture between counting Disjunctions and the Statements in one Edge. <br>
 * So we count here the disjunctions (inherited in the edge) and the number of statements used. Every edge has the
 * following rating formula: <br>
...
public class DisjunctMultiStatementRating implements IRating {
	 ...
	 */
	DisjunctMultiStatementRating(final IMinimizedEdge edge) {
		if (edge.isBasicEdge()) {
			ratingValue =
					new RatingValueContainer<>(new Integer[] { 0, edge.getElementCount(), edge.getElementCount() });
		} else {
			if (!(edge instanceof ICompositeEdge)) {
				throw new IllegalArgumentException("There should be an CompositeEdge here!");
			}
			...
			for (final IMinimizedEdge compEdge : edges) {
				final Integer[] ratingValues = (Integer[]) compEdge.getRating().getRatingValueContainer().getValue();
			...
			ratingValue =
					new RatingValueContainer<>(new Integer[] { totalDisjunctions, totalStatements, computedRating });
		}
	}
	 ...
	 *
	 * @param prefValue
	 *            only the rating value, disjunctions or statements doesn't matter
	 */
	public DisjunctMultiStatementRating(final String prefValue) {
		ratingValue = new RatingValueContainer<>(new Integer[] { 0, 0, Integer.parseInt(prefValue) });
	}
	...
	public int compareTo(final IRating other) {
		...
		final Integer[] values = ((DisjunctMultiStatementRating) other).getRatingValueContainer().getValue();
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.uni_freiburg.informatik.ultimate.blockencoding.rating.interfaces.IRating #getRatingValueContainer()
	 */
	@Override
