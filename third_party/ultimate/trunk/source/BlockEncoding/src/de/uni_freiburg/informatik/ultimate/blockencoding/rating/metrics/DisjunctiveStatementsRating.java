...
/**
 * Here we store the amount of disjunctions on one edge and additionally store for each disjunction the amount of
 * statements inside. <br>
 * We can compare these ratings, since we implement Comparable. The logic is that we do not count disjunctions, which
 * have an amount of elements which under a certain boundary. In addition we rate disjunctions which are over a certain
 * amount twice as high.
 *
...
public class DisjunctiveStatementsRating implements IRating {
	...
	/**
	 * to store the values we use here a Map<DisjunctionEdge, Integer>, so for every Disjunction (inherited) we count
	 * the amount of statements in it. The size of the determines the amount of disjunctions, in general.
	 */
	...
	DisjunctiveStatementsRating(final IMinimizedEdge edge) {
		...
		if (edge.isBasicEdge()) {
			...
			valueMap.putAll(((DisjunctiveStatementsRating) left.getRating()).getRatingValueContainer().getValue());
			// Consequently add the map also from the right
			valueMap.putAll(((DisjunctiveStatementsRating) right.getRating()).getRatingValueContainer().getValue());
			...
			value = new RatingValueContainer<>(valueMap);
		} else {
			throw new IllegalArgumentException("Edge is not BasicEdge neither CompositeEdge");
		}
	}
	 ...
	 */
	public DisjunctiveStatementsRating(final String prefValue) {
		...
		if (prefs.length != 3) {
			throw new IllegalArgumentException("Preference String should" + " contain exactly three items!");
		}
		...
		final HashMap<DisjunctionEdge, Integer> fakeMap = new HashMap<>();
		for (int i = 0; i < countOfDisjunctions; i++) {
			fakeMap.put(new DisjunctionEdge(), ((underStmtBound + upperStmtBound) / 2));
		}
		value = new RatingValueContainer<>(fakeMap);
	}
	...
	public int compareTo(final IRating other) {
		...
		int otherDisjunctions = otherRating.getRatingValueContainer().getValue().size();
		...
		for (final DisjunctionEdge edge : otherRating.getRatingValueContainer().getValue().keySet()) {
			if (otherRating.getRatingValueContainer().getValue().get(edge) <= underStmtBound) {
				otherDisjunctions--;
			} else if (otherRating.getRatingValueContainer().getValue().get(edge) > upperStmtBound) {
				otherDisjunctions++;
			}
		}
		// we compare simply the count of disjunctions, according our logic
		return Integer.compare(thisDisjunctions, otherDisjunctions);
	}
	 ...
	 */
	public void setUnderStmtBound(final int underStmtBound) {
	 ...
	 */
	public void setUpperStmtBound(final int upperStmtBound) {
