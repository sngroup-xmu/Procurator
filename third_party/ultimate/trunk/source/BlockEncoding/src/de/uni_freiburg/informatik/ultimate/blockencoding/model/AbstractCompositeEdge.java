...
/**
 * Basic abstract class for composite edges (Conjunction or Disjunction), because their behavior is here exactly the
 * same.
 *
...
public abstract class AbstractCompositeEdge implements ICompositeEdge {
	...
	/**
	 * This edge (exactly the formula of this edge) is the left side of the operation
	 */
	protected IMinimizedEdge leftEdge;

	/**
	 * This edge (exactly the formula of this edge) is the right side of the operation
	 */
	 ...
	 */
	protected AbstractCompositeEdge(final IMinimizedEdge left, final IMinimizedEdge right) {
		...
		if (left instanceof AbstractCompositeEdge) {
			mContainedDisjunctions = ((AbstractCompositeEdge) left).getContainedDisjunctions();
		}
		if (right instanceof AbstractCompositeEdge) {
			mContainedDisjunctions += ((AbstractCompositeEdge) right).getContainedDisjunctions();
		}
	 ...
	 * @return
	 */
	private HashSet<IMinimizedEdge> initContainedEdges(final IMinimizedEdge left, final IMinimizedEdge right) {
		final HashSet<IMinimizedEdge> conEdges = new HashSet<>();
		if (left.isBasicEdge()) {
			conEdges.add(left);
		} else {
			conEdges.addAll(((AbstractCompositeEdge) left).getContainedEdgesSet());
		}
		if (right.isBasicEdge()) {
			conEdges.add(right);
		} else {
			conEdges.addAll(((AbstractCompositeEdge) right).getContainedEdgesSet());
		}
		return conEdges;
	}
	...
	private boolean duplicationOfFormulaRecursive(final IMinimizedEdge edgeToCheck) {
		...
		final IMinimizedEdge[] compositeEdges = ((ICompositeEdge) edgeToCheck).getCompositeEdges();
	...
	/**
	 * This method checks whether we duplicate parts of our formula. In general there should be no duplication, but
	 * there exist examples, BigBranchingExample, where this can happen. If we have a duplication, we not allow the
	 ...
	 */
	@Override
	public boolean duplicationOfFormula(final IMinimizedEdge edgeToCheck) {
		...
		if (!edgeToCheck.isBasicEdge()) {
			final HashSet<IMinimizedEdge> interSection = new HashSet<>(containedEdges);
			interSection.retainAll(((AbstractCompositeEdge) edgeToCheck).getContainedEdgesSet());
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.uni_freiburg.informatik.ultimate.blockencoding.interfaces.model. IMinimizedEdge#getSource()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.uni_freiburg.informatik.ultimate.blockencoding.interfaces.model. IMinimizedEdge#getTarget()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.uni_freiburg.informatik.ultimate.blockencoding.interfaces.model. IMinimizedEdge#isBasicEdge()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.uni_freiburg.informatik.ultimate.blockencoding.interfaces.model. ICompositeEdge#getCompositeEdges()
	 */
	@Override
	...
	@Override
	public void setTarget(final MinimizedNode target) {
		rightEdge.setTarget(target);
	}

	@Override
	public void setSource(final MinimizedNode source) {
	...
	@Override
	public void redirectTarget(final MinimizedNode target) {
		throw new UnsupportedOperationException();
	}

	@Override
	public void redirectSource(final MinimizedNode source) {
		throw new UnsupportedOperationException();
	}
	...
	public List<IWalkable> getSuccessors() {
		return (List) Arrays.asList(leftEdge, rightEdge);
	}
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.uni_freiburg.informatik.ultimate.blockencoding.model.interfaces. IMinimizedEdge#isOldVarInvolved()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.uni_freiburg.informatik.ultimate.blockencoding.model.interfaces. IMinimizedEdge#getDifferentVariables()
	 */
	@Override
