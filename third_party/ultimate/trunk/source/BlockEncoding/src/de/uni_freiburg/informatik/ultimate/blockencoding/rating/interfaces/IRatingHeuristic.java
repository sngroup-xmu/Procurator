...
/**
 * Basically classes which implement this interface define a certain boundary for which we take another rated edge
 * level. The main target is, that this boundary is automatically defined (computed) so that no user input should be
...
public interface IRatingHeuristic {
	...
	/**
	 * This method determines, if the certain boundary, defined by this heuristic, is reached.
	 *
	 * @param rating
	 *            the Rating to check
	 ...
	 */
	boolean isRatingBoundReached(IRating rating, List<IMinimizedEdge> edgeLevel);

	/**
	 * The heuristic can be initialized with a preference value which can be determined in the global preferences of the
	 * plugin.
	 *
	 * @param givenPref
	 *            the preference value as String
	 */
	void init(String givenPref);
	 ...
	 */
	boolean isRatingStrategySupported(RatingStrategy strategy);
