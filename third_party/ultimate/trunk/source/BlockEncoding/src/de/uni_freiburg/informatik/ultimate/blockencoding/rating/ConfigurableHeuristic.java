...
/**
 * Basically this "heuristic" takes the values, which are specified on the preference page.
 *
...
public class ConfigurableHeuristic implements IRatingHeuristic {
	 ...
	 */
	public ConfigurableHeuristic(final RatingStrategy strategy) {
		mStrategy = strategy;
	}
	...
	public void init(final String givenPref) {
		...
		switch (mStrategy) {
		case LARGE_BLOCK:
			throw new IllegalArgumentException("For Large Block Encoding, there is no heuristic needed");
		case DEFAULT:
		...
		default:
			throw new IllegalArgumentException(
					"Unkown state of the enum RatingStrategy," + " need to place all possibilites here!");
		}
	}
