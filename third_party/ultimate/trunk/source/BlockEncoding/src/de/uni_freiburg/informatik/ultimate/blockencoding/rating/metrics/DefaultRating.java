...
/**
 * The default rating, simply counts the number of statements which are inside a specific minimized edge.
 *
...
public class DefaultRating implements IRating {
	...
	DefaultRating(final IMinimizedEdge edge) {
		...
		if (edge instanceof ICompositeEdge) {
			...
			if (!(left.getRating() instanceof DefaultRating) || !(right.getRating() instanceof DefaultRating)) {
			...
			mCountOfStatements.setValue(
					leftRating.getRatingValueContainer().getValue() + rightRating.getRatingValueContainer().getValue());
		}
	}

	/**
	 * Constructor to create a rating for a heuristic, should be used only for this operation. To protect it from misuse
	 * the visibility is default.
	 *
	 * @param value
	 *            the preference value
	 */
	public DefaultRating(final String prefValue) {
		// Here we interpret the preference string
		mCountOfStatements = new RatingValueContainer<>(Integer.parseInt(prefValue));
	}
	...
	public int compareTo(final IRating other) {
		...
		return mCountOfStatements.getValue().compareTo(otherRating.getRatingValueContainer().getValue());
	}
