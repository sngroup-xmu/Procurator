...
/**
 * In this rating we count the number of used variables, this may be an interesting value to measure the complexity of
 * single formulas.
 *
...
public class UsedVariablesRating implements IRating {
	 ...
	 */
	UsedVariablesRating(final IMinimizedEdge edge) {
		countUsedVariables = new RatingValueContainer<>(edge.getDifferentVariables().size());
	}
	 ...
	 */
	public UsedVariablesRating(final String prefValue) {
		countUsedVariables = new RatingValueContainer<>(Integer.parseInt(prefValue));
	}
	...
	public int compareTo(final IRating other) {
		...
		return countUsedVariables.getValue()
				.compareTo(((UsedVariablesRating) other).getRatingValueContainer().getValue());
	}
