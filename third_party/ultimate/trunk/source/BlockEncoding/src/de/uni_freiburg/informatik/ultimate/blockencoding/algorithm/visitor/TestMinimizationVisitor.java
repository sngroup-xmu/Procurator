...
/**
 * This visitor is used to validate the minimization results. It should iterate over the minimized graph and check if he
 * could apply a minimization rule. If this is possible, we can say that the minimization is not complete.
 *
...
public class TestMinimizationVisitor extends AbstractMinimizationVisitor {
	...
	public TestMinimizationVisitor(final ILogger logger) {
		super(logger);
	}
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.uni_freiburg.informatik.ultimate.blockencoding.algorithm. AbstractRCFGVisitor
	 * #applyMinimizationRules(de.uni_freiburg.informatik.ultimate .blockencoding.model.MinimizedNode)
	 */
	@Override
	...
	private boolean checkIfSequentialMergePossible(final MinimizedNode node) {
		...
		if (checkForMultipleSequentialMerge(node)) {
			return true;
		} else if (node.getIncomingEdges().size() == 1 && node.getOutgoingEdges().size() == 1) {
	...
	private boolean checkForMultipleSequentialMerge(final MinimizedNode node) {
		// In this run there can be nodes with one incoming and two outgoing
		// edges, which we also want to merge
	...
	private boolean checkIfParallelMergePossible(final MinimizedNode node) {
