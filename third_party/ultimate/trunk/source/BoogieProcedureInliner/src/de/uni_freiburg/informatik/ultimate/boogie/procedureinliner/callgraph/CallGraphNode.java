...
/**
 * Node of a Boogie call graph. A Node represents a procedure, where the directed edges are the calls inside this
 * procedure. Multiple calls to the same procedure occur multiple times. The order of all calls is preserved.
 *
...
public class CallGraphNode extends ModifiableLabeledEdgesMultigraph<CallGraphNode, CallGraphEdgeLabel> {
	...
	public CallGraphNode(final String id) {
		mId = id;
	}
	...
	public void setProcedureWithSpecification(final Procedure p) {
		assert p == null || p.getIdentifier().equals(mId);
		assert p == null || p.getSpecification() != null;
		mProcedureWithSpecification = p;
	}

	public void setProcedureWithBody(final Procedure p) {
		assert p == null || p.getIdentifier().equals(mId);
		assert p == null || p.getBody() != null;
		mProcedureWithBody = p;
	}

	public void setLabel(final CallGraphNodeLabel label) {
		mLabel = label;
	}
	...
	/**
	 * @return The procedure is declared and implemented in the same object. {@link #getProcedureWithSpecification()}
	 *         and {@link #getProcedureWithBody()} will return the same object;
	 */
