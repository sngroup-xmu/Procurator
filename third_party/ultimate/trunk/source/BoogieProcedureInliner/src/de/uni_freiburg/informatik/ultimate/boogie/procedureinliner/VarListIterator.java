...
/**
 * Iterates over multiple groups of VarLists at the same time. All groups must have the same number of variables, but
 * the partitioning of the groups into VarLists can differ.
 *
...
public class VarListIterator {
	...
	/**
	 * mVarListIndices[group][var] contains the index of the VarList inside "group"-th group that contains the "var"-th
	 ...
	 *
	 * @param varListGroups
	 *            Groups of VarLists. All groups must have the same total number of variables.
	 */
	public VarListIterator(final VarList[]... varListGroups) {
	...
	/**
	 * Advance to the next element(s) of this iterator. Don't call this
	 */
	public void next() {
		if (hasNext()) {
			++mCursor;
	...
	public String currentId(final int group) {
		checkCursorBounds();
		return currentVarList(group).getIdentifiers()[mIdIndices[group][mCursor]];
	}

	public VarList currentVarList(final int group) {
		checkCursorBounds();
	...
	public boolean varListChanged(final int group) {
		if (mCursor > 0) {
