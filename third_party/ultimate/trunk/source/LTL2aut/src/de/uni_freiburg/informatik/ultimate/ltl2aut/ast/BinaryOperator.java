...
public class BinaryOperator extends AstNode {
	...
	public BinaryOperator(final BinaryType type) {
		this.type = type;
	}

	public BinaryOperator(final BinaryType type, final AstNode left, final AstNode right) {
		this.type = type;
	...
	public String toString() {
		...
		for (; i < getOutgoingNodes().size() - 1; i++) {
			result.append(getOutgoingNodes().get(i).toString()).append(op);
		}
		result.append(getOutgoingNodes().get(i).toString());
		result.append(" )");
