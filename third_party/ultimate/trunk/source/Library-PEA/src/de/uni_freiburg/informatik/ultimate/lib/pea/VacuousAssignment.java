...
public class VacuousAssignment extends TCSWriter {
	...
	private Vector<CDD> possibleVacuousMaker; // list of formulas, such that the pea this formulas was generated from,
	...
	public void getVacuousAssignments(final PhaseEventAutomata pea) throws IOException {
		...
		if (numberOfLocations == 1) {
			...
			for (int i = 0; i < numberOfInitStates; i++) {
				final Phase initState = init.get(0);
	...
	public void getVacuousAssignments(final PhaseEventAutomata pea, final String reqName) throws IOException {
		...
		//// this.writer.newLine();
		//// System.out.println("PEA "+pea.getName()+" has only 1 state with an atomic proposition thus it cannot become
		// vacuously satisfied");
		//// }
		//// else {
