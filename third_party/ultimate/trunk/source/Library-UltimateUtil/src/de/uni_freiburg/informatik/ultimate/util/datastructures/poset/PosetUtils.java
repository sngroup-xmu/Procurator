...
public class PosetUtils {
	...
	/**
	 * Return true iff there is no element in allElement that is strictly greater than element with respect to
	 * {@link IPartialComparator} comp.
	 */
	public static <T> boolean isMaximalElement(final T elem, final Stream<T> allElems,
			final IPartialComparator<T> comp) {
		final boolean result = allElems.allMatch(x -> comp.compare(x, elem) != ComparisonResult.STRICTLY_GREATER);
		return result;
	}

	/**
	 * Return true iff there is no element in allElement that is strictly smaller than element with respect to
	 * {@link IPartialComparator} comp.
	 */
	public static <T> boolean isMinimalElement(final T eleme, final Stream<T> allElems,
			final IPartialComparator<T> comp) {
	...
	/**
	 * Returns a Stream that contains only the maximal elements with respect to {@link IPartialComparator} comp.
	 */
	...
	/**
	 * Returns a Stream that contains only the minimal elements with respect to {@link IPartialComparator} comp.
	 */
