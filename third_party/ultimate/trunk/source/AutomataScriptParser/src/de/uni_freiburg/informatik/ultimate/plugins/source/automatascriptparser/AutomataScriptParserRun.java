...
public class AutomataScriptParserRun {
	...
	public AutomataScriptParserRun(final IUltimateServiceProvider services, final ILogger logger, final Reader reader,
			final String filename, final String fileAbsolutePath) {
		mServices = services;
		mLogger = logger;
		mReader = reader;
		mFilename = filename;
		...
		if (result instanceof AutomataTestFileAST) {
			...
			if (parser.containsOtherAutomataFilesToParse()) {
				final String baseDir =
						parser.getFilePath().substring(0, parser.getFilePath().lastIndexOf(File.separator) + 1);
				final List<AutomatonAST> automataDefinitionsFromOtherFiles =
						parseAutomataDefinitions(parser.getFilesToParse(), baseDir);
				...
				for (final AutomatonAST a : automataDefinitionsFromOtherFiles) {
					if (autDefs.hasAutomaton(a)) {
						mLogger.debug(
								"Automaton \"" + a.getName() + "\" was already declared in file \"" + filename + "\".");
					} else {
	...
	/**
	 * Parses automata definitions from given files. This is usually required, if in a automata test file (.ats file)
	 * the parseAutomata(pathToAutomataDefinitions) is called.
	 *
	 * @param filesToParse
	...
	private List<AutomatonAST> parseAutomataDefinitions(final List<String> filesToParse, final String baseDir) {
		final List<AutomatonAST> parsedAutomata = new ArrayList<>();
		for (final String fileToParse : filesToParse) {
			...
			if ((result != null) && (result instanceof AutomataTestFileAST)) {
				...
				if (!ats.getAutomataDefinitions().isEmpty()) {
					final List<AutomatonAST> newAutomataDefinitions =
							ats.getAutomataDefinitions().getListOfAutomataDefinitions();
	...
	/**
	 * Opens a file, if the file exists and can be opened, and returns a file object, which is ready to read.
	 *
	 * @param fileName
	 *            the file to open
	 * @param baseDir
	...
	/**
	 * Checks if the given path contains file separators and returns the file separator in case it contains any.
	 *
	 * @param path
	...
	/**
	 * Adapts file separators of the given path, if they are different to current OS file separator. It maintains
	 ...
	 */
	private void reportSyntaxError(final Severity sev, final String longMessage, final String shortMessage,
			final ILocation loc) {
		if (mServices == null) {
