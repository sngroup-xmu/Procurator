...
public class NestedwordAST extends AtsASTNode {
	...
	public NestedwordAST(final ILocation loc) {
		super(loc);
		mCallPositions = new ArrayDeque<>();
		mWord = new ArrayList<>();
		mNestingRelation = new ArrayList<>();
		msizeOfWordSymbols = 0;
		setType(de.uni_freiburg.informatik.ultimate.automata.nestedword.NestedWord.class);
	}

	public void addSymbol(final CallSymbolAST c) {
	...
	public void addSymbol(final InternalSymbolAST c) {
		mWord.add(c.getSymbol());
	...
	public void addSymbol(final ReturnSymbolAST c) {
	...
	/**
	 * Checks if this Nestedword is correct in the sense, that - its NestingRelation values are in range, - it does not
	 * contain any crossing edges and - the nesting relation is symmetric
	 *
	...
	public boolean isNestedWordCorrect() {
		...
		return (nestingRelationValuesInRange(nestingRelation) && nestingEdgesDoNotCross(nestingRelation)
				&& nestingRelationSymmetricNestingEdges(nestingRelation));
	}
	...
	/*
	 * FIXME: Following methods are copied from nwalibrary.NestedWord, maybe they should be made public in NestedWord,
	 * so I can use them.
	 */

	/**
	 * Checks if an int array is a possible candidate for a nesting relation. This method is only used in assertions.
	 *
	 * @param nestingRelation
	 *            Our array model of a nesting relation
	 * @return True iff every entry of nestingRelation is in index in the range of the array or an INTERNAL_POSITION,
	 *         PLUS_INFINITY or MINUS_INFINITY.
	 */
	private boolean nestingRelationValuesInRange(final int[] nestingRelation) {
		for (final int element : nestingRelation) {
			if (element == INTERNAL_POSITION) {
			} else if (0 <= element && element < nestingRelation.length) {
			} else if (element == PLUS_INFINITY) {
			} else if (element == MINUS_INFINITY) {
			} else {
				return false;
			}
		}
	 ...
	 * @param nestingRelation
	 *            Our array model of a nesting relation
	 * @return True iff nestingRelation[i]=j implies nestingRelation[j]=i for all i such that 0<=nestingRelation[i]<
	 *         nestingRelation.length
	 */
	 ...
	 *
	 * @param nestingRelation
	 * @return False iff the modeled nesting relation contains (i,j) and (i',j') such that i<i'<=j<j'.
	 */
