...
public class ConditionalBooleanExpressionAST extends AtsASTNode {
	...
	public ConditionalBooleanExpressionAST(final ILocation loc) {
		super(loc);
		mReturnType = Boolean.class;
		mExpectingType = mReturnType;
	}

	public ConditionalBooleanExpressionAST(final ILocation loc, final AtsASTNode element) {
		super(loc);
		mReturnType = Boolean.class;
		mExpectingType = mReturnType;
	...
	public ConditionalBooleanExpressionAST(final ILocation loc, final AtsASTNode element1, final AtsASTNode element2) {
		super(loc);
		mReturnType = Boolean.class;
		mExpectingType = mReturnType;
	...
	public void setOperator(final ConditionalBooleanOperatorAST operator) {
		moperator = operator;
	}
	...
	public String getAsString() {
		if (mChildren.size() == 2) {
			...
			return mChildren.get(0).getAsString() + " " + operatorAsString + " " + mChildren.get(1).getAsString();
