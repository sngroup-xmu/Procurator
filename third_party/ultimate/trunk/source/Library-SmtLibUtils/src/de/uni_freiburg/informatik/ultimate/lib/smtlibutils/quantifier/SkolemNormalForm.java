...
/**
 * Takes a formula in prenex normal form (see {@link PrenexNormalForm}) and computes a formula in Skolem normal form.
 *
...
public class SkolemNormalForm {
	...
	public SkolemNormalForm(final ManagedScript mgdScript, final Term inputFormula) {
		...
		assert new PrenexNormalForm(mgdScript).transform(inputFormula) == inputFormula
				: "input formula must be in " + "prenex normal form";
	...
	private void skolemize(final QuantifiedFormula inputFormula) {
		...
		for (final QuantifiedVariables qb : qs.getQuantifierBlocks()) {
			if (qb.getQuantifier() == QuantifiedFormula.FORALL) {
				universalQuantifiersInScope = new QuantifiedVariables(QuantifiedFormula.FORALL,
						DataStructureUtils.union(qb.getVariables(), universalQuantifiersInScope.getVariables()));
			} else {
				...
				final Term[] universalVarsArray = universalVars.toArray(new Term[universalVars.size()]);

				final List<Sort> paramSortList =
						universalVars.stream().map(tv -> tv.getSort()).collect(Collectors.toList());
				...
				for (final TermVariable existVar : qb.getVariables()) {
					...
					final String skolemFunctionName = getFreshFunctionSymbol(paramSorts, resultSort);
