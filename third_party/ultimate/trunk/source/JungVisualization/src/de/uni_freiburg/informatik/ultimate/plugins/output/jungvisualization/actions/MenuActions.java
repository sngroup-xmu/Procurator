...
public class MenuActions implements IWorkbenchWindowActionDelegate {
	...
	/**
	 * Sets the working mode in the system propery MenuActions.SYSTEmMODE. Is called when a user switches between
	 * Picking and Transforming modes. Synchronizes radio buttons in the context menu with those in the main menu.
	 *
	 * @param mode
	 *            String either MenuActions.MODE_PICKING or MenuActions.MODE_TRANSFORMING
	 */
	public void setMode(final JungEditorInput editorInput, final Mode mode) {
	 ...
	 */
	public void changeMode(final IAction action, final JungEditorInput editorInput) {
		assert editorInput != null;
		...
		} else if (action.getId().endsWith("TransformingMode") && action.isChecked()) {
