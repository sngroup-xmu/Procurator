 ...
 * </pre>
 *
 * for ordinals α1 > ... > αk and natural numbers n1, ..., nk.
 *
 * Ordinals are immutable.
 *
 * WARNING: this class is not part of functional code and has not been properly debugged!
 *
 * @author Jan Leike
...
public final class Ordinal implements Comparable<Ordinal> {
	...
	private Ordinal(final Ordinal exponent) {
		mcomponents = Collections.singletonList(new Component(BigInteger.ONE, exponent));
	}
	...
	public boolean isFinite() {
		return isZero() || (mcomponents.size() == 1 && mcomponents.get(0).exponent.isZero());
	}
	...
	/**
	 * @return whether this is a limit ordinal (there is no ordinal β such that this is β + 1)
	 */
	...
	/**
	 * Computes the sum of this with another ordinal Warning: this operation is *not* commutative.
	 */
	public Ordinal add(final Ordinal o) {
		/*
		 * Let a = sum{i=1..k} omega^{ai} * ni, b = sum{i=1..q} omega^{bi} * mi be ordinals in the Cantor normal form.
		 * Let r be the index such that ar = b1 (if the term with the exponent b1 is not present in the Cantor normal
		 * form of a, it should be added into its proper place with the zero coefficient nr). Then sum{i=1..r-1}
		 * omega^{ai}*ni + omega^{b1}*(nr+m1) + sum{j=2..q} omega^{bj}*mj is the Cantor normal form of a + b.
		 */
		...
		if (last != null && last.exponent.compareTo(oexp) == 0) {
			result.add(new Component(last.base.add(o.mcomponents.get(0).base), oexp));
	...
	/**
	 * Computes the product of this with another ordinal Warning: this operation is *not* commutative.
	 */
	public Ordinal mult(final Ordinal o) {
		/*
		 * Let a = sum{i=1..k} omega^{ai}*ni, b = sum{j=1..q-1} omega^{bj}*mj + m be ordinals in the Cantor normal form.
		 * Then the Cantor normal form of a * b is sum{j=1..q-1} omega^{a1+bj}*mj + omega^{a1}*n1*m + sum{i=2..k}
		 * omega^{ai}*ni if m > 0, or sum{j=1..q-1} omega^{a1+bj}*mj if m = 0.
		 */
		if (isZero()) {
			return ZERO;
		}
