...
/**
 * A composed lexicographic template is a ranking template that corresponds to a lexicographic ranking function where
...
public class ComposedLexicographicTemplate extends ComposableTemplate {
	 ...
	 *
	 * @param parts
	 *            the ranking templates that are used as lexicographic entries
	 */
	...
	public List<List<LinearInequality>> getConstraintsDec(final Map<IProgramVar, TermVariable> inVars,
		...
		final List<List<List<List<LinearInequality>>>> constraints = new ArrayList<>();

		// \/_i T_i^<
		{
			final List<List<List<LinearInequality>>> disjunction = new ArrayList<>();
			for (final ComposableTemplate t : mParts) {
		...
		final List<List<LinearInequality>> cnf = TemplateComposition.distribute(constraints);
	...
	private List<String> blankAnnotations(final int size) {
		final List<String> annotations = new ArrayList<>(size);
