...
/**
 * The lexicographic template finds lexicographic ranking functions where each entry is an affine-linear function.
 *
 * Template:
 *
 * <pre>
	...
	public List<List<LinearInequality>> getConstraints(final Map<IProgramVar, TermVariable> inVars,
		...
		for (int i = 0; i < size - 1; ++i) {
			...
			for (int j = i - 1; j >= 0; --j) {
				...
				li.add(a);
				li.setStrict(true);
				li.mMotzkinCoefficient = RED_ATOMS && j == 0 ? PossibleMotzkinCoefficients.ZERO_AND_ONE
		...
		for (int i = 0; i < size; ++i) {
			...
			li.setStrict(true);
			li.mMotzkinCoefficient =
					(RED_ATOMS && i == 0) || (BLUE_ATOMS && i == size - 1) ? PossibleMotzkinCoefficients.ZERO_AND_ONE
							: PossibleMotzkinCoefficients.ANYTHING;
			disjunction.add(li);
		}
		conjunction.add(disjunction);
