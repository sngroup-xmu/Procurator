...
public final class TemplateComposition {
	/**
	 * Build a CNF out of a /\ \/ /\ \/ -formula of linear inequalities using the following rule:
	 *
	 * <pre>
	 ...
	 * @return a CNF
	 */
	static <T> List<List<T>> distribute(final List<List<List<List<T>>>> constraints) {
		final List<List<T>> conjunction = new ArrayList<>();
		for (final List<List<List<T>>> i : constraints) {
			...
			while (true) {
				final List<T> disjunction = new ArrayList<>();
				for (int j = 0; j < f.length; ++j) {
	 ...
	 *            the constraints
	 */
	static void resetMotzkin(final List<List<LinearInequality>> constraints) {
	 ...
	 */
	static int computeDegree(final List<List<LinearInequality>> constraints) {
		int degree = 0;
		for (final List<LinearInequality> disjunction : constraints) {
			for (final LinearInequality li : disjunction) {
