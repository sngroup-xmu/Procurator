...
/**
 * The multiphase template finds ranking functions that proceed through a fixed number of phases where each phase is
 * ranked by an affine-linear function.
 *
 * Template:
 *
 * <pre>
...
public class MultiphaseTemplate extends ComposableTemplate {
		...
		for (int i = 0; i < size; ++i) {
			...
			if (i > 0) {
				...
				li3.mMotzkinCoefficient =
						BLUE_ATOMS ? PossibleMotzkinCoefficients.ZERO_AND_ONE : PossibleMotzkinCoefficients.ANYTHING;
				disjunction.add(li3);
			}
	...
	public List<List<LinearInequality>> getConstraintsBounded(final Map<IProgramVar, TermVariable> inVars,
		...
		for (int i = 0; i < size; ++i) {
			...
			li.setStrict(true);
			li.mMotzkinCoefficient =
					(i == 0 && RED_ATOMS) || (i > 0 && BLUE_ATOMS) ? PossibleMotzkinCoefficients.ZERO_AND_ONE
