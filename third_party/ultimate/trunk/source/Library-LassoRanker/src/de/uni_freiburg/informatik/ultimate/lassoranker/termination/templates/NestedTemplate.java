...
/**
 * The nested template finds ranking functions that decrease each iteration by at least the negated value of another
 * function that is also decreasing.
 *
 * This seems to be a special case of the Polyranking principle introduced by Bradley, Sipma and Manna. We still have to
 * investigate the exact relation.
 *
 * <pre>
 *    δ > 0
 * /\ ( f_0(x') < f_0(x) - δ )
...
public class NestedTemplate extends ComposableTemplate {
	...
	/**
	 * @param functions
	 *            number of linear functions in the nested template
	 */
	...
	protected void init() {
		...
		for (int i = 0; i < mSize; ++i) {
			mfgens[i] =
					new AffineFunctionGenerator(mScript, mVariables, s_name_function + getInstanceNumber() + "_" + i);
		}
	}
	...
	@Override
	public List<List<LinearInequality>> getConstraintsDec(final Map<IProgramVar, TermVariable> inVars,
	...
	@Override
	public List<List<LinearInequality>> getConstraintsNonInc(final Map<IProgramVar, TermVariable> inVars,
			final Map<IProgramVar, TermVariable> outVars) {
		final List<List<LinearInequality>> conjunction = new ArrayList<>();
		// /\_i f_i(x') ≤ f_i(x)
		for (int i = 0; i < mSize; ++i) {
	...
	@Override
	public List<List<LinearInequality>> getConstraintsBounded(final Map<IProgramVar, TermVariable> inVars,
			final Map<IProgramVar, TermVariable> outVars) {
		// f_n(x) > 0
	...
	public List<String> getAnnotationsBounded() {
		return Collections.singletonList("rank f_" + (mSize - 1) + " is bounded");
	}
}
