...
public class EquivalentCells {
	...
	public EquivalentCells(final Script script, final ModifiableTransFormula tf,
			final List<ArrayEquality> arrayEqualities, final List<ArrayUpdate> arrayUpdates,
			final IndexAnalysisResult indexAnalyzer,
			final Map<TermVariable, Map<ArrayIndex, TermVariable>> arrayInstance2Index2CellVariable) {
		mScript = script;
		mTransFormula = tf;
	...
	private void addArrayUpdateConstraints(final UnionFind<TermVariable> uf, final List<ArrayUpdate> arrayUpdates) {
		for (final ArrayUpdate au : arrayUpdates) {
			...
			final Map<ArrayIndex, TermVariable> newInstance2Index2CellVariable =
					mArrayInstance2Index2CellVariable.get(au.getNewArray());
			final Map<ArrayIndex, TermVariable> oldInstance2Index2CellVariable =
					mArrayInstance2Index2CellVariable.get(au.getOldArray());
	...
	private void addArrayEqualityConstraints(final UnionFind<TermVariable> uf,
			final List<ArrayEquality> arrayEqualities) {
		for (final ArrayEquality ae : arrayEqualities) {
			final Map<ArrayIndex, TermVariable> lhsInstance2Index2CellVariable =
					mArrayInstance2Index2CellVariable.get(ae.getLhsTermVariable());
			final Map<ArrayIndex, TermVariable> rhsInstance2Index2CellVariable =
					mArrayInstance2Index2CellVariable.get(ae.getRhsTermVariable());
	...
	private Term computeInOutEqualities(final UnionFind<TermVariable> unionFind) {
		...
		for (final TermVariable representative : unionFind.getAllRepresentatives()) {
			final List<TermVariable> equalInOutVars = new ArrayList<>();
			for (final TermVariable member : unionFind.getEquivalenceClassMembers(representative)) {
				if (ModifiableTransFormulaUtils.isInVar(member, mTransFormula)
						|| ModifiableTransFormulaUtils.isOutVar(member, mTransFormula)) {
	...
	private TermVariable computeInOutRepresentative(final UnionFind<TermVariable> uf,
			final TermVariable ufRepresentative) {
