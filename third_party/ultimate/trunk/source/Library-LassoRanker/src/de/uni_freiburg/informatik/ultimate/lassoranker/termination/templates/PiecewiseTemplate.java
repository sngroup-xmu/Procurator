...
/**
 * The piecewise template finds ranking functions that are piecewise defined functions.
 *
 * Template:
 *
 * <pre>
 ...
 * </pre>
 *
 * The functions f_i define the pieces. Each function has a discriminatory predicate g_i() >= 0. We use this piece iff
 * the corresponding predicate is satisfied.
 *
 * @author Jan Leike
 */
public class PiecewiseTemplate extends RankingTemplate {
	...
	protected void init() {
		...
		for (int i = 0; i < size; ++i) {
			mfgens[i] = new AffineFunctionGenerator(mScript, mVariables, s_name_function + i);
			mpgens[i] = new AffineFunctionGenerator(mScript, mVariables, s_name_pred + i);
		}
	}
	...
	@Override
	public List<List<LinearInequality>> getConstraints(final Map<IProgramVar, TermVariable> inVars,
			final Map<IProgramVar, TermVariable> outVars) {
		checkInitialized();
		final List<List<LinearInequality>> conjunction = new ArrayList<>();

		// /\_i /\_j g_i(x) < 0 \/ g_j(x') < 0 \/ f_j(x') < f_i(x) - delta
		for (int i = 0; i < size; ++i) {
			for (int j = 0; j < size; ++j) {
				final List<LinearInequality> disjunction = new ArrayList<>();

				final LinearInequality li1 = mpgens[i].generate(inVars);
				li1.negate();
				li1.setStrict(true);
				li1.mMotzkinCoefficient = BLUE_ATOMS && i == j ? PossibleMotzkinCoefficients.ZERO_AND_ONE
				...
				li3.add(a);
				li3.setStrict(true);
				li3.mMotzkinCoefficient =
						RED_ATOMS ? PossibleMotzkinCoefficients.ZERO_AND_ONE : PossibleMotzkinCoefficients.ANYTHING;
				disjunction.add(li3);
		...
		for (int i = 0; i < size; ++i) {
			...
			li.setStrict(true);
			li.mMotzkinCoefficient = RED_ATOMS ? PossibleMotzkinCoefficients.ONE : PossibleMotzkinCoefficients.ANYTHING;
		...
		for (int i = 0; i < size; ++i) {
			...
			li.setStrict(false);
			li.mMotzkinCoefficient = RED_ATOMS && i == 0 ? PossibleMotzkinCoefficients.ZERO_AND_ONE
					: PossibleMotzkinCoefficients.ANYTHING;
			disjunction.add(li);
		}
		conjunction.add(disjunction);
	...
	public Collection<Term> getCoefficients() {
		final Collection<Term> list = new ArrayList<>();
		list.add(mdelta);
		for (int i = 0; i < size; ++i) {
	...
	@Override
	public RankingFunction extractRankingFunction(final Map<Term, Rational> val) throws SMTLIBException {
