...
public class ReachDefRCFGPredecessorGenerator extends RCFGEdgeVisitor {
	...
	public ReachDefRCFGPredecessorGenerator(final IAnnotationProvider<ReachDefStatementAnnotation> provider,
	 ...
	 * @return
	 */
	public List<ReachDefStatementAnnotation> process(final IcfgLocation currentNode) {
		rtr = new ArrayList<>();
		if (currentNode == null) {
			return rtr;
		}
		...
		if (mLogger.isDebugEnabled()) {
			mLogger.debug("Predecessors: " + Util.prettyPrintIterable(currentNode.getIncomingEdges(),
					Util.<IcfgEdge> createHashCodePrinter()));
		}
	...
	@Override
	protected void visit(final StatementSequence stmtSeq) {
		final ReachDefStatementAnnotation annot =
				mProvider.getAnnotation(stmtSeq.getStatements().get(stmtSeq.getStatements().size() - 1));
		if (annot != null) {
			rtr.add(annot);
		}
