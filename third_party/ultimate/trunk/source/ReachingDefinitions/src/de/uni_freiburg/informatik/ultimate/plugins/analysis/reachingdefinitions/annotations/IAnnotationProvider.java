...
public interface IAnnotationProvider<T> {
	...
	T getAnnotation(IElement element);

	T getAnnotation(IElement element, String uniqueId);

	/**
	 * Return all annotations that were made with this provider for this element regardless of key
	 */
	List<T> getAllAnnotations(IElement element);

	void annotate(IElement node, T annotation);

	void annotate(IElement node, T annotation, String uniqueId);
