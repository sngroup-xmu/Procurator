...
class ComputeMemlocInitializingTransformula {
	...
	ComputeMemlocInitializingTransformula(final MemlocArrayManager memlocArrayManager, final IProgramVar validArray, // final
																														// UnmodifiableTransFormula
																														// originalTransFormula,
		...
		computeInitializingTerm(Objects.requireNonNull(memlocArrayManager), Objects.requireNonNull(validArray));
		mResult = computeInitializingTransformula();
	}

	private void computeInitializingTerm(final MemlocArrayManager memlocArrayManager, final IProgramVar validArray) {
		...
		for (final IProgramNonOldVar locArray : globalLocArrays) {
			...
			final TermVariable memlocArrayTv =
					mMgdScript.constructFreshTermVariable(locArray.getGloballyUniqueId(), locArray.getSort());
	...
	public UnmodifiableTransFormula computeInitializingTransformula() {
		/*
		 * steps: <li> add a conjunct with equalities l_x_frz = lit_l_x_frz (l means a location) to the Transformula's
		 * formula <li> add all l_x_frz as assigned vars to the invars/outvars
		 */
		...
		/*
		 * Note that the symbol table will be automatically updated with the new constants by the TransformedIcfgBuilder
		 * (in its .finish() method).
		 */
		final Set<IProgramConst> newNonTheoryConstants = new HashSet<>(mMemlocArrayManager.getInitLocLits());

		final TransFormulaBuilder newTfBuilder = new TransFormulaBuilder(newInVars, newOutVars,
				newNonTheoryConstants.isEmpty(), newNonTheoryConstants, true, null, true);
