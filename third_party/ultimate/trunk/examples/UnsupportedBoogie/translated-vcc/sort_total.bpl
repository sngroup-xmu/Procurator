...
implementation sort(P#x: int, P#l: int) returns ($result: int)
{
  ...
  anon11:
    ...
    assume $local_value_is($s, #tok$1^4.1, #loc.x, $ptr_to_int($ptr(^^i4, P#x)), $ptr_to(^^i4)) && $local_value_is_ptr($s, #tok$1^4.1, #loc.x, $ptr(^^i4, P#x), $ptr_to(^^i4));
    assume $local_value_is($s, #tok$1^4.1, #loc.l, P#l, ^^u4);
    assume #wrTime$1^4.1 == $current_timestamp($s);
    assume (forall #p: $ptr :: { $in_writes_at(#wrTime$1^4.1, #p) } $in_writes_at(#wrTime$1^4.1, #p) == $set_in(#p, $array_range($s, $ptr(^^i4, P#x), ^^i4, P#l)));
    ...
    // uint32_t j; 
    assume $local_value_is($s, #tok$1^11.2, #loc.j, L#j, ^^u4);
    // uint32_t i; 
    assume $local_value_is($s, #tok$1^10.2, #loc.i, L#i, ^^u4);
      ...
      invariant L#i >= 0 && L#i < P#l;
    {
      anon10:
        ...
        assume true;
        assume $full_stop_ext(#tok$1^13.2, $s);
        assume $local_value_is($s, #tok$1^13.2, #loc.i, L#i, ^^u4);
        assume $local_value_is($s, #tok$1^13.2, #loc.j, L#j, ^^u4);
        assume $local_value_is($s, #tok$1^13.2, #loc.temp, L#temp, ^^i4);
        assume $local_value_is($s, #tok$1^13.2, #loc.l, P#l, ^^u4);
        assume $local_value_is($s, #tok$1^13.2, #loc.x, $ptr_to_int($ptr(^^i4, P#x)), $ptr_to(^^i4)) && $local_value_is_ptr($s, #tok$1^13.2, #loc.x, $ptr(^^i4, P#x), $ptr_to(^^i4));
        ...
        if (0 < L#i)
        {
          anon7:
            // j := 0; 
            L#j := 0;
            assume $local_value_is($s, #tok$1^17.3, #loc.j, L#j, ^^u4);
            loopState#1 := $s;
            while (true)
              invariant L#j >= 0 && L#j < P#l;
            {
              anon6:
                assume (forall #p: $ptr :: {:weight 0} { $mem($s, #p) } $irrelevant(old($s), #p) || $set_in(#p, $array_range(old($s), $ptr(^^i4, P#x), ^^i4, P#l)) || $mem_eq(old($s), $s, #p)) && (forall #p: $ptr :: {:weight 0} { $st($s, #p) } $irrelevant(old($s), #p) || $set_in(#p, $array_range(old($s), $ptr(^^i4, P#x), ^^i4, P#l)) || $st_eq(old($s), $s, #p)) && (forall #p: $ptr :: {:weight 0} { $ts($s, #p) } $irrelevant(old($s), #p) || $set_in(#p, $array_range(old($s), $ptr(^^i4, P#x), ^^i4, P#l)) || $ts_eq(old($s), $s, #p)) && $timestamp_post(old($s), $s);
                ...
                assume $local_value_is($s, #tok$1^18.3, #loc.i, L#i, ^^u4);
                assume $local_value_is($s, #tok$1^18.3, #loc.j, L#j, ^^u4);
                assume $local_value_is($s, #tok$1^18.3, #loc.temp, L#temp, ^^i4);
                assume $local_value_is($s, #tok$1^18.3, #loc.l, P#l, ^^u4);
                assume $local_value_is($s, #tok$1^18.3, #loc.x, $ptr_to_int($ptr(^^i4, P#x)), $ptr_to(^^i4)) && $local_value_is_ptr($s, #tok$1^18.3, #loc.x, $ptr(^^i4, P#x), $ptr_to(^^i4));
                // assume @_vcc_meta_eq(old(@prestate, @state), @state); 
                ...
                if (L#j < L#i)
                {
                  anon3:
                    ...
                    if ($mem($s, $idx($ptr(^^i4, P#x), L#j + 1, ^^i4)) < $mem($s, $idx($ptr(^^i4, P#x), L#j, ^^i4)))
                    {
                      anon1:
                        ...
                        assume $local_value_is($s, #tok$1^23.5, #loc.temp, L#temp, ^^i4);
                        // assert @_vcc_typed2(@state, x[j]); 
                        assert $typed2($s, $idx($ptr(^^i4, P#x), L#j, ^^i4), ^^i4);
                        // assert @prim_writes_check(x[j]); 
                        assert $writable($s, #wrTime$1^4.1, $idx($ptr(^^i4, P#x), L#j, ^^i4));
                        // assert @reads_check_normal(x[+(j, 1)]); 
                        assert $typed2($s, $idx($ptr(^^i4, P#x), L#j + 1, ^^i4), ^^i4);
                        assert $thread_local2($s, $idx($ptr(^^i4, P#x), L#j + 1, ^^i4), ^^i4);
                        // assert @in_range_u4(+(j, 1)); 
                        ...
                        assume $full_stop_ext(#tok$1^24.5, $s);
                        // assert @_vcc_typed2(@state, x[+(j, 1)]); 
                        assert $typed2($s, $idx($ptr(^^i4, P#x), L#j + 1, ^^i4), ^^i4);
                        // assert @prim_writes_check(x[+(j, 1)]); 
                        assert $writable($s, #wrTime$1^4.1, $idx($ptr(^^i4, P#x), L#j + 1, ^^i4));
                        // assert @in_range_u4(+(j, 1)); 
                        assert $in_range_u4(L#j + 1);
                        // *x[+(j, 1)] := temp; 
                  ...
                  anon4:
                    ...
                    // j := +(j, 1); 
                    L#j := L#j + 1;
                    assume $local_value_is($s, #tok$1^27.4, #loc.j, L#j, ^^u4);
                }
                else
                {
                  anon5:
          ...
          #break_2:
            ...
            // i := -(i, 1); 
            L#i := L#i - 1;
            assume $local_value_is($s, #tok$1^29.3, #loc.i, L#i, ^^u4);
        }
        else
        {
          anon9:
...
const unique #tok$1^29.3: $token;

const unique #tok$1^27.4: $token;

const unique #tok$1^25.5: $token;

const unique #tok$1^24.5: $token;
...
const unique #file^C?3A?5CUsers?5Cmaus?5CDocuments?5CUltimate2?5Cexamples?5CVCC?5Csort.cpp: $token;

axiom $file_name_is(1, #file^C?3A?5CUsers?5Cmaus?5CDocuments?5CUltimate2?5Cexamples?5CVCC?5Csort.cpp);
