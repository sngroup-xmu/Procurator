...
from p4b.paths import tofino_include_candidates
...
class P4BTranslator:
    """
    Wrapper for a P4->Boogie translator (e.g., external p4c translator).
...
def _find_tofino_include_dir(anchor: Path) -> Optional[Path]:
    ...
    for cand in tofino_include_candidates(anchor):
