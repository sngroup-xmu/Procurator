...
class TestCliCommon(unittest.TestCase):
    def test_find_default_p4b_bin_prefers_verify_backend(self) -> None:
        root = repo_root()
        prefer = root / "third_party" / "p4c" / "source" / "build-host" / "backends" / "verify" / "p4c-translator"
        legacy = root / "third_party" / "p4c" / "source" / "build-host" / "p4c-translator"
        ...
        docker = root / "src" / "dslc" / "toolchain" / "p4b_docker.sh"
        if docker.exists():
