...
class TestSpecRegressions(unittest.TestCase):
    @staticmethod
    def _bench_spec(name: str) -> Path:
        repo_root = next(p for p in Path(__file__).resolve().parents if (p / "src" / "dslc").exists())
        return repo_root / "benchmarks" / "specs" / "bench" / name
    ...
    def test_p4xos_majority_quorum_spec_avoids_else_if_and_parses(self) -> None:
        ...
        spec_path = self._bench_spec("p4xos_majority_quorum_bug.prop")
    ...
    def test_distcache_p2c_spec_guards_table_execution(self) -> None:
        ...
        spec_path = self._bench_spec("distcache_bug.prop")
        text = spec_path.read_text(encoding="utf-8", errors="replace")
    ...
    def test_distcache_cm34_write_bug_uses_node_prefixed_instrumentation_flags(self) -> None:
        ...
        spec_path = self._bench_spec("distcache_cm34_write_bug.prop")
        text = spec_path.read_text(encoding="utf-8", errors="replace")
    ...
    def test_distcache_cache_frequency_wraparound_spec_is_functional_two_phase(self) -> None:
        ...
        spec_path = self._bench_spec("distcache_leaf_cache_frequency_wraparound.prop")
        text = spec_path.read_text(encoding="utf-8", errors="replace")
