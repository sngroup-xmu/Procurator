...
class TestBoogieBackendSmoke(unittest.TestCase):
    ...
    @classmethod
    def _bench_spec(cls, name: str) -> Path:
        return cls._repo_root() / "benchmarks" / "specs" / "bench" / name

    @classmethod
    def _p4b_bin(cls) -> Path:
        return (
            cls._repo_root()
            / "third_party"
            / "p4c"
...
"""
    ...
    def test_exact_register_mirror_assert_enables_p4b_fail_fast(self) -> None:
        spec = self._bench_spec("external_flowrest_per_flow_pkt_len_total_wraparound_direct.prop")
        self.assertTrue(spec.exists())
        p4b_bin = self._p4b_bin()
    ...
    def test_guarded_register_mirror_assert_does_not_enable_fail_fast(self) -> None:
        spec = self._bench_spec("external_flowrest_per_flow_pkt_len_total_wraparound.prop")
        self.assertTrue(spec.exists())
        p4b_bin = self._p4b_bin()
        self.assertTrue(p4b_bin.exists())
    ...
    def test_guarded_register_mirror_assert_gets_dsl_direct_check(self) -> None:
        spec = self._bench_spec("external_flowrest_per_flow_flow_duration_wraparound.prop")
        self.assertTrue(spec.exists())
        p4b_bin = self._p4b_bin()
        self.assertTrue(p4b_bin.exists())
    ...
    def test_conjunctive_guarded_register_mirror_assert_gets_dsl_direct_check(self) -> None:
        spec = self._bench_spec("external_flowrest_per_flow_flow_duration_wraparound_direct.prop")
        self.assertTrue(spec.exists())
        p4b_bin = self._p4b_bin()
        self.assertTrue(p4b_bin.exists())
...
"""
    ...
    def test_required_env_packet_vars_are_kept_without_widening_p4_slice(self) -> None:
        spec = self._bench_spec("external_etc_noms2024_pkt_len_total_wraparound_slot0.prop")
        self.assertTrue(spec.exists())
        p4b_bin = self._p4b_bin()
        self.assertTrue(p4b_bin.exists())
