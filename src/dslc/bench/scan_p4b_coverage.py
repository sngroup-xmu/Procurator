...
DEFAULT_SCAN_ROOTS = (Path("benchmarks/datasets"),)
TOPLEVEL_PATTERNS = (
...
def _include_paths(root: Path, p4: Path) -> list[Path]:
    ...
    p4include = root / "third_party" / "p4c" / "source" / "p4include"
    if p4include.is_dir():
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    ap.add_argument("--scan-root", action="append", default=[], help="Root to scan for *.p4 files. Can be repeated. Defaults to benchmarks/datasets.")
    ...
    if p4b_bin is None:
        raise SystemExit("p4c-translator not found; build p4c translator or pass --p4b-bin")
