...
def _derive_compile_cmd(
    ...
    *,
    ...
    # verify cmd shape: ./src/bin/procurator verify <flags> --spec <spec>
...
def _spec_has_8bit_register(root: Path, spec_path: str) -> Tuple[bool, List[str]]:
    ...
    def _scan_include_closure(start: Path) -> List[str]:
        include_roots = [
            start.parent,
            (root / "third_party" / "p4c" / "source" / "p4include"),
        ]
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if ns.run_8bit_wrap_trials:
        trial_specs = [
            ...
            "benchmarks/specs/bench/atp_count_mismatch_bug.prop",
            "benchmarks/specs/bench/ddosd_window_label_collision_bug.prop",
        ]
